...
const ChatRoom = ({ team, user, onBack }) => {
  ...
  useEffect(() => {
    if (!supabase) {
      ...
      setMessages([{ id: 1, author: "System", text: `Welcome to the ${team.title} team chat!`, time: new Date().toISOString() }]);
      setLoading(false);
      return;
    }
    const load = async () => {
  ...
  const send = async () => {
    if (!text.trim()) return;
    const msg = { room_id: roomId, author: user.name, text: text.trim(), time: new Date().toISOString() };
    setText("");
    if (!supabase) {
      setMessages(prev => [...prev, { ...msg, id: Date.now() }]);
      return;
    }
    const { error } = await supabase.from("messages").insert([{ ...msg, created_at: msg.time }]);
    if (error) setError(error.message);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: C.bg }}>
      {/* Header */}
      <div style={{ padding: "14px 16px", background: C.white, borderBottom: `1px solid ${C.gray100}`, display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}>
          <Icon n="arrowLeft" size={20} />
        </button>
        <div style={{ flex: 1 }}>
          <p style={{ fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 700, margin: 0 }}>{team.title}</p>
          <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>{team.teams} member{team.teams !== 1 ? "s" : ""} · Team Chat</p>
        </div>
      ...
      {!supabase && (
        <div style={{ background: C.amberBg, padding: "8px 16px", display: "flex", gap: 6, alignItems: "center" }}>
          <Icon n="info" size={13} color={C.amber} />
          <p style={{ fontSize: 11, color: C.amber, margin: 0 }}>Demo mode — add Supabase env vars for persistent chat</p>
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 10 }}>
        {loading ? (
          <div style={{ textAlign: "center", paddingTop: 40 }}><Spinner size={24} /></div>
        ) : messages.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px 20px", color: C.gray400 }}>
            <Icon n="send" size={28} color={C.gray200} />
            <p style={{ marginTop: 10, fontSize: 13 }}>No messages yet. Start the conversation!</p>
          </div>
        ) : (
          messages.map((m, i) => {
            ...
            return (
              <div key={m.id || i} style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
                {!isMe && <p style={{ fontSize: 10, fontWeight: 700, color: C.gray400, marginBottom: 3, paddingLeft: 4 }}>{m.author}</p>}
                <div style={{
                  maxWidth: "78%", padding: "10px 14px", borderRadius: isMe ? "14px 14px 4px 14px" : "14px 14px 14px 4px",
                  background: isMe ? C.black : C.white,
                  color: isMe ? "#fff" : C.black,
                  border: isMe ? "none" : `1px solid ${C.gray100}`,
                  fontSize: 14, lineHeight: 1.5,
                }}>
                  {m.text}
                </div>
                <p style={{ fontSize: 10, color: C.gray400, marginTop: 3, paddingLeft: 4, paddingRight: 4 }}>{fmt(m.time || m.created_at)}</p>
              </div>
            );
          })
        )}
        {error && <p style={{ fontSize: 12, color: C.red, textAlign: "center" }}>{error}</p>}
        <div ref={bottomRef} />
      </div>

      {/* Input */}
      <div style={{ padding: "12px 16px", background: C.white, borderTop: `1px solid ${C.gray100}`, display: "flex", gap: 8, flexShrink: 0, paddingBottom: "calc(12px + env(safe-area-inset-bottom, 0px))" }}>
        <input
          value={text}
          onChange={e => setText(e.target.value)}
          onKeyDown={e => e.key === "Enter" && send()}
          placeholder="Type a message..."
          style={{ flex: 1, padding: "11px 14px", borderRadius: 22, border: `1.5px solid ${C.gray200}`, fontSize: 14, background: C.gray50, outline: "none" }}
        />
        <button className="btn" onClick={send} disabled={!text.trim()} style={{ width: 44, height: 44, borderRadius: "50%", background: text.trim() ? C.black : C.gray200, border: "none", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          {teams.map((p, i) => (
            <div key={i} className="card" style={{ background: C.white, borderRadius: 14, padding: 16, border: `1px solid ${C.gray100}` }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button className="btn" onClick={() => setChatTeam(p)} style={{ background: C.black, color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
