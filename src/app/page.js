...
// ── Direct Harvest Page ───────────────────────────────────────────────────────
const DirectHarvestPage = ({ onBack, user }) => {
  const [view, setView] = useState("home"); // home | farmer | buyer | addCrop
  const [crops, setCrops] = useState([
    { id: 1, name: "Paddy (Rice)", farmer: "Rajan K", district: "Thanjavur", qty: "500 kg", price: "₹22/kg", available: true, ts: Date.now() - 3600000 },
    { id: 2, name: "Banana",       farmer: "Muthu S", district: "Trichy",    qty: "200 kg", price: "₹18/kg", available: true, ts: Date.now() - 7200000 },
    { id: 3, name: "Tomato",       farmer: "Selvi R", district: "Coimbatore",qty: "300 kg", price: "₹14/kg", available: true, ts: Date.now() - 1800000 },
    { id: 4, name: "Sugarcane",    farmer: "Arjun M", district: "Erode",     qty: "1 ton",  price: "₹3200/ton", available: true, ts: Date.now() - 5400000 },
  ]);
  const [form, setForm] = useState({ name: "", qty: "", price: "", district: "", contact: "" });
  ...
  const getAiPrice = async () => {
    ...
    try {
      const res = await fetch("/api/groq", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: [{ role: "user", content: `What is the current fair market price for ${form.name} in Tamil Nadu? Give a short answer with price range per kg or unit. Keep it under 2 lines.` }],
          userName: user?.name || "Farmer",
        }),
      });
      const data = await res.json();
      setAiPrice(data.reply || "");
    } catch { setAiPrice("Could not fetch price. Check internet."); }
    finally { setAiLoading(false); }
  };

  const addCrop = () => {
    if (!form.name || !form.qty || !form.price) return;
    setCrops(prev => [{ id: Date.now(), name: form.name, farmer: user?.name || "You", district: form.district || "Your District", qty: form.qty, price: form.price, available: true, ts: Date.now() }, ...prev]);
    setForm({ name: "", qty: "", price: "", district: "", contact: "" });
    setAiPrice("");
    setSubmitted(true);
    setTimeout(() => { setSubmitted(false); setView("farmer"); }, 1800);
  };
  ...
  return (
    <div style={{ padding: "0 0 90px", fontFamily: "Nunito, sans-serif", background: C.bg, minHeight: "100vh" }}>
      {/* Top Bar */}
      <div style={{ background: C.black, padding: "16px 20px", display: "flex", alignItems: "center", gap: 14 }}>
        <button onClick={view === "home" ? onBack : () => setView("home")} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}>
          <Icon n="back" size={22} color="#fff" />
        </button>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 16, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Direct Harvest</p>
          <p style={{ fontSize: 11, color: C.green, margin: 0 }}>No middlemen — 98% price to farmer</p>
        </div>
        <span style={{ fontSize: 10, fontWeight: 700, background: C.green, color: "#fff", padding: "3px 9px", borderRadius: 6 }}>Farmers</span>
      </div>

      {/* Home View */}
      {view === "home" && (
        <div style={{ padding: "20px 16px 0" }}>
          <div style={{ background: C.black, borderRadius: 16, padding: "20px 18px", marginBottom: 20 }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: C.green, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>How it works</p>
            <h2 style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 10, lineHeight: 1.3 }}>Sell Crops Directly.<br />Zero Middlemen.</h2>
            <p style={{ fontSize: 13, color: C.gray400, lineHeight: 1.6, marginBottom: 16 }}>List your crop → Buyer contacts you → You sell directly. GKFXL takes only 2% commission. You keep 98% of the price.</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {[{ n: "98%", l: "To Farmer" }, { n: "2%", l: "GKFXL" }, { n: "0", l: "Middlemen" }].map((s, i) => (
                <div key={i} style={{ background: "#ffffff10", borderRadius: 10, padding: "10px 6px", textAlign: "center" }}>
                  <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 800, color: C.green, margin: "0 0 2px" }}>{s.n}</p>
                  <p style={{ fontSize: 10, color: C.gray400, margin: 0 }}>{s.l}</p>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            <button onClick={() => setView("farmer")} style={{ background: C.white, border: `1px solid ${C.gray100}`, borderRadius: 14, padding: "18px 14px", cursor: "pointer", textAlign: "left" }}>
              <div style={{ width: 40, height: 40, background: C.black, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
                <Icon n="leaf" size={18} color="#fff" />
              </div>
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 4px" }}>I am a Farmer</p>
              <p style={{ fontSize: 11, color: C.gray400, margin: 0, lineHeight: 1.4 }}>List my crops for sale</p>
            </button>
            <button onClick={() => setView("buyer")} style={{ background: C.white, border: `1px solid ${C.gray100}`, borderRadius: 14, padding: "18px 14px", cursor: "pointer", textAlign: "left" }}>
              <div style={{ width: 40, height: 40, background: C.black, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
                <Icon n="users" size={18} color="#fff" />
              </div>
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 4px" }}>I am a Buyer</p>
              <p style={{ fontSize: 11, color: C.gray400, margin: 0, lineHeight: 1.4 }}>Browse available crops</p>
            </button>
          </div>

          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, marginBottom: 12 }}>Latest Listings</p>
          {crops.slice(0, 3).map((c) => (
            <div key={c.id} style={{ background: C.white, borderRadius: 14, padding: "14px 16px", border: `1px solid ${C.gray100}`, marginBottom: 10, display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 42, height: 42, background: C.black, borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <Icon n="leaf" size={18} color="#fff" />
              </div>
              <div style={{ flex: 1 }}>
                <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 2px" }}>{c.name}</p>
                <p style={{ fontSize: 12, color: C.gray400, margin: 0 }}>{c.district} · {c.qty} · {timeAgo(c.ts)}</p>
              </div>
              <p style={{ fontSize: 13, fontWeight: 800, color: C.green, margin: 0, flexShrink: 0 }}>{c.price}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {view === "farmer" && (
        <div style={{ padding: "20px 16px 0" }}>
          <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
            <button onClick={() => setView("addCrop")} style={{ flex: 1, background: C.black, color: "#fff", border: "none", borderRadius: 12, padding: "13px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              <Icon n="leaf" size={16} color="#fff" /> List New Crop
            </button>
          </div>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, marginBottom: 12 }}>All Listings ({crops.length})</p>
          {crops.map((c) => (
            <div key={c.id} style={{ background: C.white, borderRadius: 14, padding: "14px 16px", border: `1px solid ${C.gray100}`, marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, margin: 0 }}>{c.name}</p>
                <span style={{ fontSize: 11, fontWeight: 700, background: C.greenBg, color: C.green, padding: "2px 8px", borderRadius: 5 }}>Active</span>
              </div>
              <p style={{ fontSize: 12, color: C.gray400, margin: "0 0 4px" }}>Farmer: {c.farmer} · {c.district}</p>
              <p style={{ fontSize: 13, fontWeight: 600, color: C.black, margin: 0 }}>{c.qty} · {c.price} · {timeAgo(c.ts)}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {view === "buyer" && (
        <div style={{ padding: "20px 16px 0" }}>
          <div style={{ background: C.greenBg, borderRadius: 12, padding: "12px 14px", border: `1px solid ${C.green}30`, marginBottom: 18 }}>
            <p style={{ fontSize: 12, color: C.green, fontWeight: 600, margin: 0 }}>Contact farmer directly — no middlemen, fair price for everyone.</p>
          </div>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, marginBottom: 12 }}>Available Crops ({crops.length})</p>
          {crops.map((c) => (
            <div key={c.id} style={{ background: C.white, borderRadius: 14, padding: "16px", border: `1px solid ${C.gray100}`, marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
                <div>
                  <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, margin: "0 0 3px" }}>{c.name}</p>
                  <p style={{ fontSize: 12, color: C.gray400, margin: 0 }}>{c.district} · {c.qty}</p>
                </div>
                <p style={{ fontSize: 15, fontWeight: 800, color: C.green, margin: 0 }}>{c.price}</p>
              </div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <p style={{ fontSize: 12, color: C.gray400, margin: 0 }}>by {c.farmer} · {timeAgo(c.ts)}</p>
                <button style={{ background: C.black, color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>Contact</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: "20px 16px 0" }}>
          {submitted ? (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              <div style={{ width: 64, height: 64, background: C.greenBg, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                <Icon n="check" size={28} color={C.green} />
              </div>
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 800, color: C.black, margin: "0 0 8px" }}>Crop Listed!</p>
              <p style={{ fontSize: 13, color: C.gray400 }}>Buyers can now contact you directly.</p>
            </div>
          ) : (
            <>
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 15, fontWeight: 700, color: C.black, marginBottom: 18 }}>List Your Crop</p>
              {[
                { label: "Crop Name *", key: "name", placeholder: "e.g. Paddy, Banana, Tomato" },
                { label: "Quantity *",  key: "qty",  placeholder: "e.g. 500 kg, 2 tons" },
                { label: "Your Price *",key: "price",placeholder: "e.g. ₹22/kg" },
                { label: "District",    key: "district", placeholder: "e.g. Thanjavur" },
                { label: "Contact No.", key: "contact",  placeholder: "10-digit mobile number" },
              ].map((f) => (
                <div key={f.key} style={{ marginBottom: 14 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: C.black, marginBottom: 6 }}>{f.label}</p>
                  <input value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.placeholder}
                    style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${C.gray200}`, fontSize: 14, fontFamily: "Nunito, sans-serif", background: C.white, outline: "none" }} />
                </div>
              ))}

              {form.name && (
                <button onClick={getAiPrice} disabled={aiLoading} style={{ width: "100%", padding: "11px", background: C.greenBg, border: `1px solid ${C.green}40`, borderRadius: 10, fontSize: 13, fontWeight: 700, color: C.green, cursor: "pointer", marginBottom: 14, fontFamily: "Poppins, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  {aiLoading ? <Spinner size={14} color={C.green} /> : <Icon n="zap" size={14} color={C.green} />}
                  {aiLoading ? "Getting AI Price..." : "Get AI Fair Price Suggestion"}
                </button>
              )}
              {aiPrice && (
                <div style={{ background: C.greenBg, borderRadius: 10, padding: "12px 14px", border: `1px solid ${C.green}30`, marginBottom: 14 }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: C.green, margin: "0 0 4px", textTransform: "uppercase" }}>AI Price Suggestion</p>
                  <p style={{ fontSize: 13, color: C.black, margin: 0, lineHeight: 1.5 }}>{aiPrice}</p>
                </div>
              )}

              <button onClick={addCrop} disabled={!form.name || !form.qty || !form.price}
                style={{ width: "100%", padding: "14px", background: !form.name || !form.qty || !form.price ? C.gray200 : C.black, color: !form.name || !form.qty || !form.price ? C.gray400 : "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: !form.name || !form.qty || !form.price ? "not-allowed" : "pointer", fontFamily: "Poppins, sans-serif" }}>
                List Crop for Sale
              </button>
            </>
          )}
...
// ── Skill Bazaar Page ───────────────────────────────────────────────────────────────────────────────
const SkillBazaarPage = ({ onBack, user }) => {
  const [view, setView] = useState("home");
  const [skills, setSkills] = useState([
    { id: 1, name: "Tailoring",       person: "Meena R",   district: "Coimbatore", price: "\u20b9300/day",   category: "Craft",    rating: 4.8, reviews: 12 },
    { id: 2, name: "Mobile Repair",   person: "Karthik S", district: "Chennai",    price: "\u20b9500/visit", category: "Tech",     rating: 4.5, reviews: 8  },
    { id: 3, name: "Tractor Driving", person: "Arjun M",   district: "Thanjavur",  price: "\u20b9800/day",   category: "Farming",  rating: 5.0, reviews: 20 },
    { id: 4, name: "Home Tuition",    person: "Priya K",   district: "Madurai",    price: "\u20b9200/hr",    category: "Teaching", rating: 4.7, reviews: 15 },
    { id: 5, name: "Cooking",         person: "Selvi N",   district: "Salem",      price: "\u20b9400/event", category: "Craft",    rating: 4.9, reviews: 6  },
    { id: 6, name: "Plumbing",        person: "Ravi T",    district: "Trichy",     price: "\u20b9600/visit", category: "Tech",     rating: 4.3, reviews: 9  },
  ]);
  const [form, setForm]           = useState({ name: "", category: "", price: "", district: "", desc: "", contact: "" });
  const [aiSuggestion, setAiSuggestion] = useState("");
  const [aiLoading, setAiLoading]       = useState(false);
  const [submitted, setSubmitted]       = useState(false);
  const [filter, setFilter]             = useState("All");

  const categories = ["All", "Tech", "Farming", "Teaching", "Craft", "Health", "Other"];
  ...
  const getAiSuggestion = async () => {
    ...
    try {
      const res = await fetch("/api/groq", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: [{ role: "user", content: `What is a fair price for "${form.name}" as a freelance skill in Tamil Nadu? Suggest 2 tips to attract buyers. Under 3 lines.` }],
          userName: user?.name || "GGE User",
        }),
      });
      const data = await res.json();
      setAiSuggestion(data.reply || "");
    } catch { setAiSuggestion("Could not fetch. Check internet."); }
    finally { setAiLoading(false); }
  };

  const addSkill = () => {
    if (!form.name || !form.price || !form.category) return;
    setSkills(prev => [{ id: Date.now(), name: form.name, person: user?.name || "You", district: form.district || "Your District", price: form.price, category: form.category, rating: 0, reviews: 0 }, ...prev]);
    setForm({ name: "", category: "", price: "", district: "", desc: "", contact: "" });
    setAiSuggestion("");
    setSubmitted(true);
    setTimeout(() => { setSubmitted(false); setView("mySkills"); }, 1800);
  };

  const Stars = ({ r }) => <span style={{ fontSize: 11, color: "#f59e0b", fontWeight: 700 }}>{"\u2605".repeat(Math.round(r))}{r > 0 ? ` ${r.toFixed(1)}` : " New"}</span>;
  ...
  return (
    <div style={{ padding: "0 0 90px", fontFamily: "Nunito, sans-serif", background: C.bg, minHeight: "100vh" }}>
      <div style={{ background: C.black, padding: "16px 20px", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 16, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Skill Bazaar</p>
          <p style={{ fontSize: 11, color: C.green, margin: 0 }}>List your skill \u00b7 Set your price \u00b7 Earn directly</p>
        </div>
        <span style={{ fontSize: 10, fontWeight: 700, background: C.green, color: "#fff", padding: "3px 9px", borderRadius: 6 }}>Earn</span>
      </div>

      {view === "home" && (
        <div style={{ padding: "20px 16px 0" }}>
          <div style={{ background: C.black, borderRadius: 16, padding: "20px 18px", marginBottom: 20 }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: C.green, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>Skill Bazaar</p>
            <h2 style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 10, lineHeight: 1.3 }}>Any Skill. Any Price.<br />Direct Earn.</h2>
            <p style={{ fontSize: 13, color: C.gray400, lineHeight: 1.6, marginBottom: 16 }}>List any skill \u2014 tailoring, driving, cooking, teaching, repair. Set your price. Buyers contact you directly. GKFXL takes only 2%.</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {[{ n: skills.length, l: "Skills Listed" }, { n: "2%", l: "Commission" }, { n: "\u20b90", l: "To Join" }].map((s, i) => (
                <div key={i} style={{ background: "#ffffff10", borderRadius: 10, padding: "10px 6px", textAlign: "center" }}>
                  <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 800, color: C.green, margin: "0 0 2px" }}>{s.n}</p>
                  <p style={{ fontSize: 10, color: C.gray400, margin: 0 }}>{s.l}</p>
                </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            <button onClick={() => setView("addSkill")} style={{ background: C.white, border: `1px solid ${C.gray100}`, borderRadius: 14, padding: "18px 14px", cursor: "pointer", textAlign: "left" }}>
              <div style={{ width: 40, height: 40, background: C.black, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
                <Icon n="rupee" size={18} color="#fff" />
              </div>
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 4px" }}>List My Skill</p>
              <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>Start earning today</p>
            </button>
            <button onClick={() => setView("browse")} style={{ background: C.white, border: `1px solid ${C.gray100}`, borderRadius: 14, padding: "18px 14px", cursor: "pointer", textAlign: "left" }}>
              <div style={{ width: 40, height: 40, background: C.black, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
                <Icon n="users" size={18} color="#fff" />
              </div>
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 4px" }}>Find a Skill</p>
              <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>Hire someone near you</p>
            </button>
          </div>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, marginBottom: 12 }}>Top Skills</p>
          {skills.slice(0, 3).map(s => (
            <div key={s.id} style={{ background: C.white, borderRadius: 14, padding: "14px 16px", border: `1px solid ${C.gray100}`, marginBottom: 10, display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 42, height: 42, background: C.black, borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <Icon n="star" size={18} color="#fff" />
              </div>
              <div style={{ flex: 1 }}>
                <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 2px" }}>{s.name}</p>
                <p style={{ fontSize: 12, color: C.gray400, margin: "0 0 2px" }}>{s.person} \u00b7 {s.district}</p>
                <Stars r={s.rating} />
              </div>
              <p style={{ fontSize: 13, fontWeight: 800, color: C.green, margin: 0 }}>{s.price}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {view === "browse" && (
        <div style={{ padding: "20px 16px 0" }}>
          <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4, marginBottom: 16 }}>
            {categories.map(cat => (
              <button key={cat} onClick={() => setFilter(cat)} style={{ flexShrink: 0, padding: "7px 14px", borderRadius: 20, border: "none", background: filter === cat ? C.black : C.white, color: filter === cat ? "#fff" : C.gray600, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>{cat}</button>
            ))}
          </div>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, marginBottom: 12 }}>{filtered.length} skills found</p>
          {filtered.map(s => (
            <div key={s.id} style={{ background: C.white, borderRadius: 14, padding: "16px", border: `1px solid ${C.gray100}`, marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                <div>
                  <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, margin: "0 0 2px" }}>{s.name}</p>
                  <p style={{ fontSize: 12, color: C.gray400, margin: "0 0 4px" }}>{s.person} \u00b7 {s.district}</p>
                  <Stars r={s.rating} />
                </div>
                <div style={{ textAlign: "right" }}>
                  <p style={{ fontSize: 14, fontWeight: 800, color: C.green, margin: "0 0 6px" }}>{s.price}</p>
                  <span style={{ fontSize: 10, fontWeight: 700, background: C.gray50, color: C.gray600, padding: "2px 8px", borderRadius: 5 }}>{s.category}</span>
                </div>
              </div>
              <button style={{ width: "100%", marginTop: 8, background: C.black, color: "#fff", border: "none", borderRadius: 9, padding: "9px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>
                Contact {s.person.split(" ")[0]}
              </button>
            </div>
          ))}
        </div>
      ...
      {view === "mySkills" && (
        <div style={{ padding: "20px 16px 0" }}>
          <button onClick={() => setView("addSkill")} style={{ width: "100%", background: C.black, color: "#fff", border: "none", borderRadius: 12, padding: "13px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 18 }}>
            <Icon n="rupee" size={16} color="#fff" /> Add New Skill
          </button>
          {skills.filter(s => s.person === (user?.name || "You")).length === 0
            ? <p style={{ textAlign: "center", fontSize: 14, color: C.gray400, padding: "40px 0" }}>No skills listed yet.</p>
            : skills.filter(s => s.person === (user?.name || "You")).map(s => (
              <div key={s.id} style={{ background: C.white, borderRadius: 14, padding: "14px 16px", border: `1px solid ${C.gray100}`, marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 700, color: C.black, margin: 0 }}>{s.name}</p>
                  <span style={{ fontSize: 11, fontWeight: 700, background: C.greenBg, color: C.green, padding: "2px 8px", borderRadius: 5 }}>Active</span>
                </div>
                <p style={{ fontSize: 13, color: C.green, fontWeight: 700, margin: "4px 0 0" }}>{s.price}</p>
              </div>
            ))
          }
        </div>
        ...
        <div style={{ padding: "20px 16px 0" }}>
          {submitted ? (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              ...
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 800, color: C.black, margin: "0 0 8px" }}>Skill Listed!</p>
              <p style={{ fontSize: 13, color: C.gray400 }}>Buyers can now contact you directly.</p>
            </div>
          ) : (
            <>
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 15, fontWeight: 700, color: C.black, marginBottom: 18 }}>List Your Skill</p>
              {[
                { label: "Skill Name *", key: "name",     placeholder: "e.g. Tailoring, Mobile Repair" },
                { label: "Your Price *", key: "price",    placeholder: "e.g. \u20b9300/day" },
                { label: "District",     key: "district", placeholder: "e.g. Coimbatore" },
                { label: "Description",  key: "desc",     placeholder: "What do you offer?" },
                { label: "Contact No.",  key: "contact",  placeholder: "10-digit mobile number" },
              ].map(f => (
                <div key={f.key} style={{ marginBottom: 14 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: C.black, marginBottom: 6 }}>{f.label}</p>
              ...
              <div style={{ marginBottom: 14 }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: C.black, marginBottom: 8 }}>Category *</p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {["Tech","Farming","Teaching","Craft","Health","Other"].map(cat => (
                    <button key={cat} onClick={() => setForm(p => ({ ...p, category: cat }))}
                      style={{ padding: "7px 14px", borderRadius: 20, border: "none", background: form.category === cat ? C.black : C.gray50, color: form.category === cat ? "#fff" : C.gray600, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>{cat}</button>
                  ))}
                </div>
              </div>
              {form.name && (
                <button onClick={getAiSuggestion} disabled={aiLoading}
                  style={{ width: "100%", padding: "11px", background: C.greenBg, border: `1px solid ${C.green}40`, borderRadius: 10, fontSize: 13, fontWeight: 700, color: C.green, cursor: "pointer", marginBottom: 14, fontFamily: "Poppins, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  {aiLoading ? <Spinner size={14} color={C.green} /> : <Icon n="zap" size={14} color={C.green} />}
                  {aiLoading ? "Getting AI Suggestion..." : "Get AI Pricing Suggestion"}
                </button>
              )}
              {aiSuggestion && (
                <div style={{ background: C.greenBg, borderRadius: 10, padding: "12px 14px", border: `1px solid ${C.green}30`, marginBottom: 14 }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: C.green, margin: "0 0 4px", textTransform: "uppercase" }}>AI Suggestion</p>
                  <p style={{ fontSize: 13, color: C.black, margin: 0, lineHeight: 1.5 }}>{aiSuggestion}</p>
                </div>
              )}
              <button onClick={addSkill} disabled={!form.name || !form.price || !form.category}
                style={{ width: "100%", padding: "14px", background: !form.name || !form.price || !form.category ? C.gray200 : C.black, color: !form.name || !form.price || !form.category ? C.gray400 : "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>
                List Skill for Sale
              </button>
            </>
          )}
...
// ── Life Reels ───────────────────────────────────────────────────────────────
// ── Static Reels Data ─────────────────────────────────────────────────────────
// Real YouTube embed IDs — public educational videos, India-relevant
const REELS = [
  {
    id: 1,
    title: "How to Save ₹500 Every Month",
    creator: "GGE Guide",
    district: "Chennai",
    category: "Money",
    tag: "All",
    ...
    shares: 892,
    views: "48K",
    desc: "Simple tricks any Indian family can use to save ₹500 every month without feeling the pinch.",
    color: "#1a3a2a",
    accent: "#2ecc71",
    emoji: "",
    tip: "Start with a ₹20/day savings jar. Most people can do this without noticing.",
  },
  {
    id: 2,
    title: "Soil Testing at Home — Free Method",
    creator: "Smart Farmer TN",
    district: "Coimbatore",
    ...
    shares: 1540,
    views: "63K",
    desc: "Test your soil health at home using just a bottle of water — no lab needed. Know if your soil is ready before spending on seeds.",
    color: "#1a2e10",
    accent: "#a3e635",
    emoji: "",
    tip: "If soil floats → sandy. If sinks fast → clay. Both need different treatment.",
  },
  {
    id: 3,
    title: "Your Rights If Police Stop You",
    creator: "Kanoon AI",
    district: "Madurai",
    category: "Legal",
    tag: "All",
    ...
    shares: 3800,
    views: "91K",
    desc: "Every Indian must know these 3 rights when stopped by police. Section 41D, 50, 57 — explained in plain Tamil and English.",
    color: "#1a1a2e",
    accent: "#818cf8",
    emoji: "️",
    tip: "You have the right to know WHY you are being arrested. Always ask politely.",
  },
  {
    id: 4,
    title: "10-Minute Daily Habit That Changes Everything",
    creator: "GGE Mental Health",
    district: "Trichy",
    ...
    shares: 2100,
    views: "76K",
    desc: "One 10-minute morning habit that reduces anxiety, improves focus, and costs ₹0. Backed by science, proven in Indian families.",
    color: "#1e1030",
    accent: "#c084fc",
    emoji: "",
    tip: "Write 3 things you are grateful for every morning. Do it for 21 days.",
  },
  {
    id: 5,
    title: "How to Get PM Kisan ₹6000 — Step by Step",
    creator: "Govt Bridge",
    district: "Thanjavur",
    category: "Government",
    tag: "Farmers",
    ...
    shares: 4300,
    views: "124K",
    desc: "Exactly how to apply for PM Kisan Samman Nidhi ₹6000/year. Documents needed, website, common mistakes to avoid.",
    color: "#1f1000",
    accent: "#f59e0b",
    emoji: "️",
    tip: "Common rejection reason: Aadhaar name mismatch with land records. Fix this first.",
  },
  {
    id: 6,
    title: "Freelance from Your Phone — ₹0 Investment",
    creator: "Career Guide",
    district: "Salem",
    category: "Career",
    tag: "Job Seekers",
    duration: "1:55",
    ...
    shares: 2760,
    views: "89K",
    desc: "3 freelance skills you can learn in 1 week and start earning from your phone. No laptop, no degree, no investment needed.",
    color: "#0f1a1f",
    accent: "#22d3ee",
    emoji: "",
    tip: "Canva design, WhatsApp business posts, and data entry are the fastest ₹0 entry skills.",
  },
  {
    id: 7,
    title: "Sugar Control Without Medicine",
    creator: "Health Guide TN",
    district: "Erode",
    category: "Health",
    tag: "All",
    duration: "2:05",
    ...
    shares: 5400,
    views: "152K",
    desc: "Tamil Nadu has the highest diabetes rate in India. These 3 daily food habits can lower blood sugar naturally — no medicine needed.",
    color: "#1f0a0a",
    accent: "#f87171",
    emoji: "",
    tip: "Replace white rice with red rice + dal combo. Blood sugar drops 20% in 30 days.",
  },
  {
    id: 8,
    title: "Start a Business for ₹500",
    creator: "GKFXL",
    district: "Vellore",
    category: "Business",
    tag: "All",
    ...
    shares: 1800,
    views: "54K",
    desc: "Real businesses you can start with just ₹500 in Tamil Nadu. Not dreams — actual steps, actual numbers, actual people who did it.",
    color: "#101f10",
    accent: "#4ade80",
    emoji: "",
    tip: "Reselling on Meesho or OLX requires ₹0. Start there before investing anything.",
  },
];

const CATEGORIES = ["All", "Money", "Agriculture", "Legal", "Mental Health", "Government", "Career", "Health", "Business"];

// ── Icon (inline SVG, no dependency) ─────────────────────────────────────────
const Ic = ({ d, size = 20, color = "currentColor", fill = "none", strokeWidth = 1.8 }) => (
...
// ── Single Reel Card (vertical, fullscreen-style) ─────────────────────────────
const ReelCard = ({ reel, liked, saved, onLike, onSave, onShare, onAsk }) => {
  ...
  const askAI = async () => {
    ...
    try {
      const res = await fetch("/api/groq", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: [{ role: "user", content: `I just watched a video titled "${reel.title}" (category: ${reel.category}). My question: "${question}". Answer in 2-3 sentences, simply and practically for an Indian audience.` }],
          language: "english",
        }),
      });
      const data = await res.json();
      setAiReply(data.reply || "Great question! Keep learning and applying what you know.");
    } catch {
      setAiReply("Great question! The key is to start small and stay consistent.");
    }
    setAiLoading(false);
  };

  return (
    <div style={{ background: reel.color, borderRadius: 20, overflow: "hidden", marginBottom: 14, position: "relative", minHeight: 480 }}>

      {/* ── Video placeholder (gradient + emoji) ── */}
      <div style={{ height: 260, background: `linear-gradient(160deg, ${reel.color}, ${reel.accent}22)`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
        {/* Decorative circles */}
        <div style={{ position: "absolute", top: -30, right: -30, width: 140, height: 140, background: reel.accent + "15", borderRadius: "50%" }} />
        <div style={{ position: "absolute", bottom: -20, left: -20, width: 100, height: 100, background: reel.accent + "10", borderRadius: "50%" }} />
        {/* Play button */}
        <div style={{ textAlign: "center", zIndex: 1 }}>
          <div style={{ fontSize: 64, marginBottom: 8 }}>{reel.emoji}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "center" }}>
            <div style={{ width: 44, height: 44, background: "rgba(255,255,255,0.15)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)", border: "1.5px solid rgba(255,255,255,0.25)" }}>
              <svg width={18} height={18} viewBox="0 0 24 24" fill="#fff" stroke="none"><polygon points="5 3 19 12 5 21 5 3"/></svg>
            </div>
            <span style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", fontWeight: 600 }}>{reel.duration}</span>
          </div>
        </div>
        {/* Duration + views */}
        <div style={{ position: "absolute", bottom: 10, right: 12, display: "flex", gap: 6 }}>
          <span style={{ fontSize: 10, background: "rgba(0,0,0,0.5)", color: "#fff", padding: "3px 8px", borderRadius: 6, fontWeight: 700 }}>{reel.views} views</span>
          <span style={{ fontSize: 10, background: reel.accent + "33", color: reel.accent, padding: "3px 8px", borderRadius: 6, fontWeight: 700, border: `1px solid ${reel.accent}50` }}>{reel.category}</span>
        </div>
        {/* Creator */}
        <div style={{ position: "absolute", top: 12, left: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(0,0,0,0.4)", borderRadius: 20, padding: "4px 10px 4px 4px", backdropFilter: "blur(4px)" }}>
            <div style={{ width: 22, height: 22, background: reel.accent, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontSize: 10, fontWeight: 900, color: reel.color }}>{reel.creator[0]}</span>
            </div>
            <span style={{ fontSize: 11, color: "#fff", fontWeight: 600 }}>{reel.creator}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "14px 16px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8, marginBottom: 8 }}>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 16, fontWeight: 800, color: "#fff", margin: 0, lineHeight: 1.3, flex: 1 }}>{reel.title}</p>
          <span style={{ fontSize: 10, color: reel.accent, fontWeight: 700, flexShrink: 0, marginTop: 3 }}> {reel.district}</span>
        </div>
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", margin: "0 0 12px", lineHeight: 1.55 }}>{reel.desc}</p>

        {/* Action row */}
        <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
          {[
            { icon: liked ? "️" : "", label: fmt(reel.likes + (liked ? 1 : 0)), action: onLike, active: liked },
            { icon: saved ? "" : "", label: fmt(reel.saves + (saved ? 1 : 0)), action: onSave, active: saved },
            { icon: "", label: fmt(reel.shares), action: onShare, active: false },
          ].map((btn, i) => (
            <button key={i} onClick={btn.action} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "9px 6px", background: btn.active ? reel.accent + "25" : "rgba(255,255,255,0.08)", border: `1px solid ${btn.active ? reel.accent + "60" : "rgba(255,255,255,0.12)"}`, borderRadius: 10, cursor: "pointer" }}>
              <span style={{ fontSize: 14 }}>{btn.icon}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: btn.active ? reel.accent : "rgba(255,255,255,0.7)" }}>{btn.label}</span>
            </button>
          ))}
        </div>

        {/* Insider Tip */}
        <button onClick={() => setShowTip(!showTip)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: "rgba(255,255,255,0.07)", border: `1px solid ${reel.accent}30`, borderRadius: 12, cursor: "pointer", marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 14 }}></span>
            <span style={{ fontSize: 12, fontWeight: 700, color: reel.accent }}>Insider Tip</span>
          </div>
          <span style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", transform: showTip ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▾</span>
        </button>
        {showTip && (
          <div style={{ background: reel.accent + "18", borderRadius: 10, padding: "10px 12px", marginBottom: 8, border: `1px solid ${reel.accent}30` }}>
            <p style={{ fontSize: 13, color: "#fff", margin: 0, lineHeight: 1.6 }}>{reel.tip}</p>
          </div>
        )}

        {/* Ask AI */}
        <button onClick={() => setAskOpen(!askOpen)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 12, cursor: "pointer" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 14 }}></span>
            <span style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.8)" }}>Ask AI about this</span>
          </div>
          <span style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", transform: askOpen ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▾</span>
        </button>
        {askOpen && (
          <div style={{ marginTop: 8, padding: "12px", background: "rgba(255,255,255,0.06)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)" }}>
            {aiReply ? (
              <div style={{ marginBottom: 10 }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: reel.accent, textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 5px" }}>GGE AI</p>
                <p style={{ fontSize: 13, color: "#fff", margin: 0, lineHeight: 1.6 }}>{aiReply}</p>
              </div>
            ) : null}
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                style={{ flex: 1, padding: "9px 12px", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, fontSize: 13, color: "#fff", outline: "none", fontFamily: "Nunito, sans-serif" }}
              />
              <button onClick={askAI} disabled={aiLoading || !question.trim()} style={{ padding: "9px 14px", background: reel.accent, border: "none", borderRadius: 10, cursor: "pointer", fontWeight: 700, fontSize: 13, color: reel.color, opacity: aiLoading ? 0.6 : 1 }}>
                {aiLoading ? "..." : "Ask"}
              </button>
            </div>
          </div>
...
// ── Saved Reels Tab ───────────────────────────────────────────────────────────
const SavedTab = ({ savedIds }) => {
  const saved = REELS.filter(r => savedIds.includes(r.id));
  if (saved.length === 0) return (
    <div style={{ textAlign: "center", padding: "60px 20px", color: C.gray400 }}>
      <p style={{ fontSize: 40, marginBottom: 12 }}></p>
      <p style={{ fontSize: 15, fontWeight: 700, color: C.black, margin: "0 0 6px" }}>No saved reels yet</p>
      <p style={{ fontSize: 13, margin: 0 }}>Tap the save button on any reel to keep it here</p>
    </div>
  );
  return (
    <div style={{ padding: "12px 16px 90px" }}>
      {saved.map(r => (
        <div key={r.id} style={{ background: r.color, borderRadius: 14, padding: "12px 14px", marginBottom: 10, display: "flex", gap: 12, alignItems: "center" }}>
          <div style={{ fontSize: 32 }}>{r.emoji}</div>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: "#fff", margin: "0 0 2px", fontFamily: "Poppins, sans-serif", lineHeight: 1.3 }}>{r.title}</p>
            <p style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", margin: 0 }}>{r.category} • {r.duration}</p>
          </div>
          <span style={{ fontSize: 10, background: r.accent + "30", color: r.accent, padding: "3px 8px", borderRadius: 6, fontWeight: 700 }}>{r.views}</span>
        </div>
      ))}
    </div>
  );
...
// ── Main LifeReelsPage ─────────────────────────────────────────────────────────
const LifeReelsPage = ({ onBack, user }) => {
  const [tab, setTab]       = useState("feed");       // feed | saved
  const [category, setCat]  = useState("All");
  const [liked, setLiked]   = useState([]);
  const [saved, setSaved]   = useState([]);

  const filtered = category === "All" ? REELS : REELS.filter(r => r.category === category);
  ...
  return (
    <div style={{ background: C.black, minHeight: "100vh", fontFamily: "Nunito, sans-serif" }}>

      {/* ── Top Bar ── */}
      <div style={{ background: "#0a0a0a", padding: "16px 20px", position: "sticky", top: 0, zIndex: 50, borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 17, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Life Reels</p>
            <p style={{ fontSize: 11, color: "#666", margin: 0 }}>2-min guidance videos — free forever</p>
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            <div style={{ background: "#1a1a1a", borderRadius: 10, padding: "5px 10px", border: "1px solid #333" }}>
              <p style={{ fontSize: 11, fontWeight: 800, color: C.gold, margin: 0 }}>{saved.length} saved</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 12 }}>
          {["feed", "saved"].map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: "8px", border: `1px solid ${tab === t ? "#fff" : "#333"}`, borderRadius: 10, background: tab === t ? "#fff" : "transparent", color: tab === t ? C.black : "#666", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif", textTransform: "capitalize" }}>
              {t === "feed" ? "For You" : `Saved (${saved.length})`}
            </button>
          ))}
        </div>
      </div>
      ...
      {tab === "saved" ? (
        <SavedTab savedIds={saved} />
      ) : (
        <>
          {/* ── Category pills ── */}
          <div style={{ overflowX: "auto", display: "flex", gap: 8, padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setCat(cat)} style={{ flexShrink: 0, padding: "6px 14px", borderRadius: 20, border: `1.5px solid ${category === cat ? "#fff" : "#333"}`, background: category === cat ? "#fff" : "transparent", color: category === cat ? C.black : "#888", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>
                {cat}
              </button>
            ))}
          </div>
          ...
          <div style={{ margin: "12px 16px 0", background: "linear-gradient(135deg, #1a3a2a, #0d1f14)", borderRadius: 14, padding: "12px 14px", border: "1px solid #2a4a3a", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 22 }}></span>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: "#fff", margin: "0 0 1px" }}>Personalised for {user?.district || "your district"}</p>
              <p style={{ fontSize: 11, color: "#4ade80", margin: 0 }}>AI picks content based on your role & location</p>
            </div>
          </div>
          ...
          <div style={{ padding: "12px 16px 90px" }}>
            {filtered.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 20px", color: "#555" }}>
                <p style={{ fontSize: 14 }}>No reels in this category yet.</p>
              </div>
            ) : filtered.map(reel => (
              <ReelCard
                key={reel.id}
                reel={reel}
                liked={liked.includes(reel.id)}
...
// ── Static Data ───────────────────────────────────────────────────────────────
const MOCK_REPS = [
  { id: 1, name: "Murugan S.", village: "Vazhavachanur", district: "Tiruvannamalai", families: 14, rating: 4.9, badge: true, specialty: "Farming & Schemes", avatar: "M", color: "#1a6b3c", active: true },
  { id: 2, name: "Kavitha R.", village: "Pappankulam",   district: "Tirunelveli",    families: 11, rating: 4.7, badge: true, specialty: "Women Rights & Health", avatar: "K", color: "#7c3aed", active: true },
  { id: 3, name: "Ravi T.",   village: "Sirugudi",       district: "Dindigul",       families: 9,  rating: 4.6, badge: false, specialty: "Digital Literacy", avatar: "R", color: "#2563eb", active: true },
];

const LOCAL_RESOURCES = [
  { id: 1, type: "scheme",   icon: "️", title: "PM Fasal Bima Yojana",        desc: "Crop insurance scheme open for kharif season. Deadline: July 31.", tag: "Farmers",  urgent: true  },
  { id: 2, type: "health",   icon: "", title: "Free Health Camp — July 15",   desc: "Govt mobile health unit visiting Vazhavachanur. BP, sugar, eye test — all free.", tag: "All",      urgent: true  },
  { id: 3, type: "job",      icon: "", title: "MGNREGA Work Available",       desc: "100 days of guaranteed work. Contact your panchayat office with Aadhaar.", tag: "Rural",    urgent: false },
  { id: 4, type: "scheme",   icon: "", title: "Jal Jeevan Mission Water",     desc: "Apply for piped water connection. Form at block office, documents: Aadhaar + ration card.", tag: "All",  urgent: false },
  { id: 5, type: "alert",    icon: "️", title: "Fake Seed Alert — Erode",      desc: "Counterfeit cotton seeds reported in Erode market. Buy only from registered dealers.", tag: "Farmers", urgent: true },
  { id: 6, type: "learning", icon: "", title: "Free Digital Training",        desc: "CSC centre offering free Tally, MS Office training. 10 seats. First come first served.", tag: "All",  urgent: false },
];

const REP_ACTIVITIES = [
  { icon: "", text: "Helped 3 families fill PM Kisan forms",   time: "Today",      pts: 150 },
  { icon: "", text: "Organised health camp for 8 families",    time: "Yesterday",  pts: 200 },
  { icon: "", text: "Taught WhatsApp UPI to 5 elders",         time: "2 days ago", pts: 100 },
  { icon: "️", text: "Guided family on land record dispute",    time: "3 days ago", pts: 120 },
  { icon: "", text: "Shared soil test results with 4 farmers", time: "4 days ago", pts: 80  },
];

const APPLY_ROLES = [
  { id: "rep",        label: "Community Rep",      desc: "Help 10+ families with govt schemes, health, digital", icon: "️" },
  { id: "agri",       label: "Agriculture Expert", desc: "Guide farmers on crops, soil, market prices",          icon: "" },
  { id: "digital",    label: "Digital Helper",     desc: "Teach elders UPI, forms, mobile basics",               icon: "" },
  { id: "women",      label: "Women Support Lead", desc: "Support women on rights, health, self-employment",     icon: "" },
];

const TAG_COLORS = {
  Farmers: { bg: "#e6f4ec", color: C.green },
  All:     { bg: C.blueBg,  color: C.blue  },
  Rural:   { bg: C.goldBg,  color: C.gold  },
};
...
const SectionHeader = ({ title, sub }) => (
  <div style={{ padding: "14px 16px 8px" }}>
    <p style={{ fontSize: 15, fontWeight: 800, color: C.black, margin: "0 0 2px", fontFamily: "Poppins, sans-serif" }}>{title}</p>
    {sub && <p style={{ fontSize: 12, color: C.gray400, margin: 0 }}>{sub}</p>}
  </div>
);

// ── Resource Card ─────────────────────────────────────────────────────────────
const ResourceCard = ({ r }) => {
  const tag = TAG_COLORS[r.tag] || TAG_COLORS.All;
  return (
    <div style={{ background: C.white, borderRadius: 14, border: `1px solid ${r.urgent ? "#fbbf24" : C.gray100}`, padding: "13px 14px", marginBottom: 10, boxShadow: r.urgent ? "0 0 0 2px #fef3c750" : "none" }}>
      <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
        <div style={{ width: 38, height: 38, background: r.urgent ? C.goldBg : C.gray50, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <span style={{ fontSize: 18 }}>{r.icon}</span>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
            {r.urgent && <span style={{ fontSize: 9, fontWeight: 800, background: "#fef3c7", color: C.gold, padding: "1px 7px", borderRadius: 5, textTransform: "uppercase", letterSpacing: 0.6 }}>Urgent</span>}
            <span style={{ fontSize: 9, fontWeight: 700, background: tag.bg, color: tag.color, padding: "1px 7px", borderRadius: 5 }}>{r.tag}</span>
          </div>
          <p style={{ fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 4px", fontFamily: "Poppins, sans-serif", lineHeight: 1.3 }}>{r.title}</p>
          <p style={{ fontSize: 12, color: C.gray600, margin: 0, lineHeight: 1.5 }}>{r.desc}</p>
        </div>
      </div>
    </div>
  );
...
// ── Rep Card ──────────────────────────────────────────────────────────────────
const RepCard = ({ rep, onContact }) => (
  <div style={{ background: C.white, borderRadius: 14, border: `1px solid ${C.gray100}`, padding: "14px", marginBottom: 10 }}>
    <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
      <div style={{ width: 44, height: 44, background: rep.color, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, position: "relative" }}>
        <span style={{ fontSize: 18, fontWeight: 900, color: "#fff" }}>{rep.avatar}</span>
        {rep.active && <div style={{ position: "absolute", bottom: 2, right: 2, width: 10, height: 10, background: "#4ade80", borderRadius: "50%", border: "2px solid #fff" }} />}
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: C.black, margin: 0, fontFamily: "Poppins, sans-serif" }}>{rep.name}</p>
          {rep.badge && <span style={{ fontSize: 11 }}></span>}
        </div>
        <p style={{ fontSize: 11, color: C.gray400, margin: "0 0 4px" }}>{rep.village}, {rep.district}</p>
        <p style={{ fontSize: 11, color: C.green, fontWeight: 600, margin: "0 0 6px" }}>{rep.specialty}</p>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 11, color: C.gold, fontWeight: 700 }}> {rep.rating}</span>
          <span style={{ fontSize: 11, color: C.gray400 }}>{rep.families} families helped</span>
        </div>
      </div>
      <button onClick={() => onContact(rep)} style={{ padding: "7px 12px", background: C.black, border: "none", borderRadius: 10, cursor: "pointer", fontSize: 12, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
        Connect
      </button>
    </div>
  </div>
...
// ── AI Guide Panel ────────────────────────────────────────────────────────────
const AIGuidePanel = ({ question, setQuestion, answer, loading, onAsk }) => (
  <div style={{ margin: "0 16px 14px", background: C.black, borderRadius: 16, padding: "16px" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
      <span style={{ fontSize: 20 }}></span>
      <div>
        <p style={{ fontSize: 13, fontWeight: 700, color: "#fff", margin: 0 }}>Ask Village AI</p>
        <p style={{ fontSize: 11, color: "#666", margin: 0 }}>Schemes • Rights • Farming • Health</p>
      </div>
    </div>
    {answer && (
      <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 12, padding: "12px", marginBottom: 10 }}>
        <p style={{ fontSize: 11, fontWeight: 700, color: C.gold, margin: "0 0 5px", textTransform: "uppercase", letterSpacing: 0.7 }}>GGE AI</p>
        <p style={{ fontSize: 13, color: "#ddd", margin: 0, lineHeight: 1.65 }}>{answer}</p>
      </div>
    )}
    <div style={{ display: "flex", gap: 8 }}>
      <input
        ...
        placeholder="E.g. How to apply for ration card?"
        style={{ flex: 1, padding: "10px 13px", background: "rgba(255,255,255,0.09)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, fontSize: 13, color: "#fff", outline: "none", fontFamily: "Nunito, sans-serif" }}
      />
      <button onClick={onAsk} disabled={loading || !question.trim()} style={{ padding: "10px 14px", background: C.green, border: "none", borderRadius: 10, cursor: "pointer", fontWeight: 700, fontSize: 13, color: "#fff", opacity: loading ? 0.6 : 1, flexShrink: 0 }}>
        {loading ? "..." : "Ask"}
      </button>
    </div>
    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
      {["Ration card apply", "PM Kisan status", "MGNREGA work", "Land record copy"].map(q => (
        <button key={q} onClick={() => { setQuestion(q); }} style={{ fontSize: 11, padding: "4px 10px", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 20, color: "#aaa", cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>
          {q}
        </button>
      ))}
    </div>
...
// ── Apply to Become Rep ───────────────────────────────────────────────────────
const ApplyRepScreen = ({ onBack }) => {
  const [form, setForm] = useState({ name: "", village: "", district: "", mobile: "", role: "", whyJoin: "" });
  const [submitted, setSubmitted] = useState(false);
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));

  const inputStyle = { width: "100%", padding: "12px 14px", border: `1.5px solid ${C.gray200}`, borderRadius: 12, fontSize: 14, fontFamily: "Nunito, sans-serif", background: C.white, color: C.black, outline: "none", boxSizing: "border-box", marginBottom: 10 };

  if (submitted) return (
    <div style={{ padding: "40px 24px", textAlign: "center" }}>
      <span style={{ fontSize: 56, display: "block", marginBottom: 16 }}></span>
      <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 20, fontWeight: 900, color: C.black, margin: "0 0 8px" }}>Application Sent!</p>
      <p style={{ fontSize: 14, color: C.gray600, lineHeight: 1.6, margin: "0 0 24px" }}>Thank you for wanting to serve your village. GKFXL will review your application within 3 days.</p>
      <div style={{ background: C.greenBg, borderRadius: 14, padding: "14px", marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: C.green, fontWeight: 700, margin: "0 0 4px" }}>What happens next?</p>
        <p style={{ fontSize: 12, color: C.gray600, margin: 0, lineHeight: 1.6 }}>If approved — you get the  GKFXL badge, +1000 bonus points, and access to rep tools.</p>
      </div>
      <button onClick={onBack} style={{ width: "100%", padding: 13, background: C.black, color: "#fff", border: "none", borderRadius: 14, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>
        Back to Village Connect
      </button>
    </div>
  );

  const valid = form.name && form.village && form.district && form.mobile.length === 10 && form.role && form.whyJoin.length >= 30;

  return (
    <div style={{ padding: "16px 16px 90px" }}>
      <div style={{ background: C.black, borderRadius: 16, padding: "16px", marginBottom: 16 }}>
        <p style={{ fontSize: 15, fontWeight: 800, color: "#fff", margin: "0 0 4px", fontFamily: "Poppins, sans-serif" }}>Apply to Serve Your Village</p>
        <p style={{ fontSize: 12, color: "#888", margin: 0 }}>One phone. One person. 10 families helped. That's the power of a community rep.</p>
      </div>

      <p style={{ fontSize: 12, fontWeight: 700, color: C.gray400, textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 8px" }}>Your Role</p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
        {APPLY_ROLES.map(r => (
          <button key={r.id} onClick={() => set("role", r.id)} style={{ padding: "12px 10px", textAlign: "left", background: form.role === r.id ? C.black : C.white, border: `1.5px solid ${form.role === r.id ? C.black : C.gray200}`, borderRadius: 12, cursor: "pointer" }}>
            <span style={{ fontSize: 22, display: "block", marginBottom: 4 }}>{r.icon}</span>
            <p style={{ fontSize: 12, fontWeight: 700, color: form.role === r.id ? "#fff" : C.black, margin: "0 0 2px", fontFamily: "Poppins, sans-serif" }}>{r.label}</p>
            <p style={{ fontSize: 10, color: form.role === r.id ? "#aaa" : C.gray400, margin: 0, lineHeight: 1.4 }}>{r.desc}</p>
          </button>
        ))}
      </div>

      <p style={{ fontSize: 12, fontWeight: 700, color: C.gray400, textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 8px" }}>Your Details</p>
      {[
        { key: "name",     placeholder: "Full Name *",           type: "text"   },
        { key: "village",  placeholder: "Village / Town *",      type: "text"   },
        { key: "district", placeholder: "District *",            type: "text"   },
        { key: "mobile",   placeholder: "Mobile Number (10 digits) *", type: "tel" },
      ].map(f => (
        <input key={f.key} type={f.type} value={form[f.key]} onChange={e => set(f.key, e.target.value)} placeholder={f.placeholder} style={inputStyle} />
      ))}
      <textarea value={form.whyJoin} onChange={e => set("whyJoin", e.target.value)} placeholder="Why do you want to help your village? (min 30 characters) *" rows={3} style={{ ...inputStyle, resize: "none" }} />

      <button onClick={() => valid && setSubmitted(true)} disabled={!valid} style={{ width: "100%", padding: 13, background: valid ? C.green : C.gray200, color: valid ? "#fff" : C.gray400, border: "none", borderRadius: 14, fontSize: 14, fontWeight: 700, cursor: valid ? "pointer" : "not-allowed", fontFamily: "Poppins, sans-serif" }}>
        Submit Application
      </button>
    </div>
  );
...
// ── Rep Activities Tab ────────────────────────────────────────────────────────
const RepActivitiesTab = () => (
  <div style={{ padding: "8px 16px 90px" }}>
    <div style={{ background: C.black, borderRadius: 14, padding: "14px 16px", marginBottom: 14 }}>
      <p style={{ fontSize: 11, fontWeight: 700, color: C.gold, textTransform: "uppercase", letterSpacing: 1, margin: "0 0 4px" }}>Community Rep — Murugan S.</p>
      <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 22, fontWeight: 900, color: "#fff", margin: "0 0 2px" }}>650 pts this week</p>
      <p style={{ fontSize: 12, color: "#888", margin: 0 }}>14 families helped • 4.9  rating</p>
    </div>
    <p style={{ fontSize: 13, fontWeight: 700, color: C.gray400, textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 10px" }}>Recent Activities</p>
    {REP_ACTIVITIES.map((a, i) => (
      <div key={i} style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.gray100}`, padding: "12px 14px", marginBottom: 8, display: "flex", gap: 10, alignItems: "center" }}>
        <div style={{ width: 36, height: 36, background: C.greenBg, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <span style={{ fontSize: 16 }}>{a.icon}</span>
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: C.black, margin: "0 0 2px", lineHeight: 1.3 }}>{a.text}</p>
          <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>{a.time}</p>
        </div>
        <span style={{ fontSize: 11, fontWeight: 800, color: C.green, background: C.greenBg, padding: "2px 8px", borderRadius: 6, flexShrink: 0 }}>+{a.pts}</span>
      </div>
    ))}
    <div style={{ background: C.greenBg, borderRadius: 14, padding: "14px 16px", marginTop: 4, border: `1px solid ${C.green}30`, textAlign: "center" }}>
      <p style={{ fontSize: 13, fontWeight: 700, color: C.green, margin: "0 0 2px" }}>Every activity earns GGE Points</p>
      <p style={{ fontSize: 12, color: C.gray600, margin: 0 }}>Top reps earn real rewards + GKFXL recognition</p>
    </div>
  </div>
);

// ── Main VillageConnectPage ───────────────────────────────────────────────────
const VillageConnectPage = ({ onBack, user }) => {
  const [tab, setTab]           = useState("feed");   // feed | reps | apply | repview
  const [aiQ, setAiQ]           = useState("");
  const [aiAns, setAiAns]       = useState("");
  const [aiLoading, setAiLoad]  = useState(false);
  const [filterTag, setFilter]  = useState("All");
  const [contactedRep, setContactedRep] = useState(null);
  ...
  const askAI = async () => {
    ...
    try {
      const res = await fetch("/api/groq", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: [{ role: "user", content: `A rural Indian villager is asking: "${aiQ}". Answer in 2-3 simple sentences. Give a practical, actionable answer. Mention specific Tamil Nadu government offices or steps if relevant.` }],
          language: "english",
        }),
      });
      const data = await res.json();
      setAiAns(data.reply || "Please visit your nearest Panchayat office or CSC (Common Service Centre) with your Aadhaar card to get started.");
    } catch {
      setAiAns("Please visit your nearest Panchayat office or CSC centre with your Aadhaar card. They can help you with most government services for free.");
    }
    setAiLoad(false);
  };

  const filtered = filterTag === "All" ? LOCAL_RESOURCES : LOCAL_RESOURCES.filter(r => r.tag === filterTag);

  if (tab === "apply") return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Nunito, sans-serif" }}>
      <div style={{ background: C.black, padding: "16px 20px", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <button onClick={() => setTab("feed")} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}>
            <BackArrow />
          </button>
          <p style={{ fontSize: 17, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Become a Rep</p>
        </div>
      </div>
      <ApplyRepScreen onBack={() => setTab("feed")} />
    </div>
  );

  if (tab === "repview") return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Nunito, sans-serif" }}>
      <div style={{ background: C.black, padding: "16px 20px", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <button onClick={() => setTab("feed")} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}>
          ...
          <p style={{ fontSize: 17, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Rep Dashboard</p>
        </div>
      </div>
      <RepActivitiesTab />
    </div>
    ...
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Nunito, sans-serif" }}>
      ...
      <div style={{ background: C.black, padding: "16px 20px", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 17, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Village Connect</p>
            <p style={{ fontSize: 11, color: "#666", margin: 0 }}>One phone. 10 families helped.</p>
          </div>
          <button onClick={() => setTab("apply")} style={{ padding: "7px 12px", background: C.green, border: "none", borderRadius: 10, cursor: "pointer", fontSize: 12, fontWeight: 700, color: "#fff" }}>
            Become Rep
          </button>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 12 }}>
          {[
            { id: "feed", label: "Local Feed" },
            { id: "reps", label: "Find Rep"  },
          ].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "8px", border: `1px solid ${tab === t.id ? "#fff" : "#333"}`, borderRadius: 10, background: tab === t.id ? "#fff" : "transparent", color: tab === t.id ? C.black : "#666", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      {tab === "feed" && (
        <>
          {/* Hero banner */}
          <div style={{ background: `linear-gradient(135deg, #0d2b1a, #1a4a2a)`, padding: "20px 16px", borderBottom: "1px solid #1f3a2a" }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
              <div style={{ width: 42, height: 42, background: "rgba(255,255,255,0.1)", borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: 22 }}>️</span>
              </div>
              <div>
                <p style={{ fontSize: 15, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Your Village Feed</p>
                <p style={{ fontSize: 11, color: "#4ade80", margin: 0 }}>Schemes • Alerts • Jobs • Health • Learning</p>
              </div>
            </div>
            {/* Quick stats */}
            <div style={{ display: "flex", gap: 8 }}>
              {[
                { n: "6",  l: "Resources"  },
                { n: "3",  l: "Active Reps" },
                { n: "38", l: "Families Helped" },
              ].map(s => (
                <div key={s.l} style={{ flex: 1, background: "rgba(255,255,255,0.07)", borderRadius: 10, padding: "8px", textAlign: "center" }}>
                  <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 900, color: "#fff", margin: "0 0 1px" }}>{s.n}</p>
                  <p style={{ fontSize: 9, color: "#4ade80", margin: 0, fontWeight: 600 }}>{s.l}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ paddingTop: 12 }}>
            <AIGuidePanel question={aiQ} setQuestion={setAiQ} answer={aiAns} loading={aiLoading} onAsk={askAI} />
          </div>

          {/* Filter pills */}
          <div style={{ display: "flex", gap: 6, padding: "0 16px 10px", overflowX: "auto" }}>
            {["All", "Farmers", "Rural", "All"].filter((v, i, a) => a.indexOf(v) === i).map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ flexShrink: 0, padding: "6px 14px", borderRadius: 20, border: `1.5px solid ${filterTag === f ? C.black : C.gray200}`, background: filterTag === f ? C.black : C.white, color: filterTag === f ? "#fff" : C.gray600, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>
                {f}
              </button>
            ))}
          </div>

          <div style={{ padding: "0 16px 16px", paddingBottom: 90 }}>
            <SectionHeader title="Resources & Alerts" sub={`${filtered.length} items for your area`} />
            {filtered.map(r => <ResourceCard key={r.id} r={r} />)}

            {/* Become a rep CTA */}
            <div style={{ background: C.black, borderRadius: 16, padding: "18px 16px", marginTop: 6 }}>
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <span style={{ fontSize: 30 }}></span>
                <div style={{ flex: 1 }}>
                  <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 15, fontWeight: 800, color: "#fff", margin: "0 0 4px" }}>Become a Community Rep</p>
                  <p style={{ fontSize: 12, color: "#888", margin: "0 0 12px", lineHeight: 1.55 }}>One person with a phone can help 10 families access schemes, health, and knowledge. Earn GGE points + GKFXL badge.</p>
                  <button onClick={() => setTab("apply")} style={{ padding: "10px 20px", background: C.green, border: "none", borderRadius: 12, fontSize: 13, fontWeight: 700, color: "#fff", cursor: "pointer" }}>
                    Apply Now — Free
                  </button>
                </div>
              </div>
      ...
      {tab === "reps" && (
        <div style={{ padding: "12px 16px 90px" }}>
          <div style={{ background: C.greenBg, borderRadius: 14, padding: "12px 14px", marginBottom: 14, border: `1px solid ${C.green}30` }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: C.green, margin: "0 0 2px" }}>Community Reps near you</p>
            <p style={{ fontSize: 12, color: C.gray600, margin: 0 }}>Tap Connect to message them directly. All reps are GKFXL verified.</p>
          </div>
          {MOCK_REPS.map(rep => (
            <RepCard key={rep.id} rep={rep} onContact={r => setContactedRep(r)} />
          ))}

          {contactedRep && (
            <div style={{ background: C.black, borderRadius: 14, padding: "14px 16px", marginTop: 4 }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: C.gold, margin: "0 0 6px" }}>Connected with {contactedRep.name}</p>
              <p style={{ fontSize: 13, color: "#ccc", margin: "0 0 10px", lineHeight: 1.55 }}>
                {contactedRep.name} from {contactedRep.village} will be notified. They typically respond within 2 hours.
              </p>
              <button onClick={() => setContactedRep(null)} style={{ padding: "8px 16px", background: C.green, border: "none", borderRadius: 10, fontSize: 12, fontWeight: 700, color: "#fff", cursor: "pointer" }}>
                OK
              </button>
            </div>
          )}

          {/* Rep dashboard preview */}
          <div style={{ background: C.gray50, borderRadius: 14, padding: "14px", marginTop: 10, border: `1px solid ${C.gray200}` }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 4px" }}>Are you already a rep?</p>
            <p style={{ fontSize: 12, color: C.gray400, margin: "0 0 10px" }}>View your activity log and earned points.</p>
            <button onClick={() => setTab("repview")} style={{ padding: "8px 16px", background: C.black, border: "none", borderRadius: 10, fontSize: 12, fontWeight: 700, color: "#fff", cursor: "pointer" }}>
              View Rep Dashboard
            </button>
          </div>
        </div>
...
// ── Family Dashboard ─────────────────────────────────────────────────────────
// ── Mock child profiles (in real app: fetched from Supabase by parent) ────────
const MOCK_CHILDREN = [
  {
    id: 1,
    name: "Priya Gokul",
    ...
    weeklyChange: +120,
    badges: [
      { id: 1, icon: "", title: "Challenge Champion", desc: "Completed 15+ challenges", earned: true },
      { id: 2, icon: "", title: "Knowledge Seeker", desc: "Completed 20+ lessons", earned: true },
      { id: 3, icon: "", title: "10-Day Streak", desc: "Active 10 days in a row", earned: true },
      { id: 4, icon: "", title: "Money Smart", desc: "Completed Money Management path", earned: true },
      { id: 5, icon: "", title: "Farmer Friend", desc: "Completed Smart Farming path", earned: false },
      { id: 6, icon: "️", title: "Rights Aware", desc: "Completed Legal Rights path", earned: false },
    ],
    learningProgress: [
      { path: "Money Management", progress: 87, color: "#f59e0b" },
      { path: "Career & Skills", progress: 62, color: "#2563eb" },
      { path: "Digital Literacy", progress: 45, color: "#7c3aed" },
      { path: "Study Skills", progress: 30, color: "#059669" },
    ],
    recentActivity: [
      { type: "lesson", text: "Completed: How to Open a Bank Account", time: "Today, 9:30 AM", pts: 50 },
      { type: "challenge", text: "Challenge Done: Track Every Rupee", time: "Today, 8:00 AM", pts: 50 },
      { type: "lesson", text: "Completed: Understanding Credit Score", time: "Yesterday, 7:45 PM", pts: 50 },
      { type: "cert", text: "Certificate Earned: Money Management", time: "2 days ago", pts: 200 },
      { type: "challenge", text: "Challenge Done: Read for 20 Minutes", time: "2 days ago", pts: 90 },
    ],
  },
  {
    id: 2,
    ...
    weeklyChange: +60,
    badges: [
      { id: 1, icon: "", title: "First Step", desc: "Completed first lesson", earned: true },
      { id: 2, icon: "", title: "3-Day Streak", desc: "Active 3 days in a row", earned: true },
      { id: 3, icon: "", title: "Knowledge Seeker", desc: "Completed 20+ lessons", earned: false },
      { id: 4, icon: "", title: "Challenge Champion", desc: "Completed 15+ challenges", earned: false },
    ],
    learningProgress: [
      { path: "Study Skills", progress: 55, color: "#059669" },
      { path: "Digital Literacy", progress: 22, color: "#7c3aed" },
    ],
    recentActivity: [
      { type: "lesson", text: "Completed: What is a Budget?", time: "Yesterday, 6:00 PM", pts: 50 },
      { type: "challenge", text: "Challenge Done: Walk 30 Minutes", time: "Yesterday, 7:30 AM", pts: 40 },
      { type: "lesson", text: "Started: Study Skills Path", time: "2 days ago", pts: 10 },
    ],
  },
];

const LEVEL_CONFIG = {
  Beginner: { color: C.gray400,  bg: C.gray50,    next: "Growing",  threshold: 200  },
  Growing:  { color: C.blue,     bg: C.blueBg,    next: "Leader",   threshold: 500  },
  Leader:   { color: C.green,    bg: C.greenBg,   next: "Champion", threshold: 800  },
  Champion: { color: C.gold,     bg: C.goldBg,    next: null,       threshold: 1000 },
};

// ── Progress Bar ──────────────────────────────────────────────────────────────
const ProgressBar = ({ value, max = 100, color = C.green, height = 8 }) => (
  <div style={{ height, background: C.gray100, borderRadius: height, overflow: "hidden" }}>
    <div style={{ height: "100%", width: `${Math.min(100, (value / max) * 100)}%`, background: color, borderRadius: height, transition: "width 0.6s ease" }} />
  </div>
);

// ── Activity Icon ─────────────────────────────────────────────────────────────
const ActivityIcon = ({ type }) => {
  const map = { lesson: { emoji: "", bg: C.blueBg }, challenge: { emoji: "", bg: C.greenBg }, cert: { emoji: "", bg: C.goldBg } };
  const s = map[type] || map.lesson;
  return (
    <div style={{ width: 34, height: 34, background: s.bg, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
const ChildSelector = ({ children, selected, onSelect }) => (
  <div style={{ display: "flex", gap: 10, padding: "12px 16px", overflowX: "auto" }}>
    {children.map(ch => (
      <button key={ch.id} onClick={() => onSelect(ch)} style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 8, padding: "8px 14px 8px 8px", background: selected.id === ch.id ? C.black : C.white, border: `1.5px solid ${selected.id === ch.id ? C.black : C.gray200}`, borderRadius: 50, cursor: "pointer", transition: "all 0.2s" }}>
        <div style={{ width: 28, height: 28, background: ch.avatarColor, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>{ch.avatar}</span>
        </div>
        <div style={{ textAlign: "left" }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: selected.id === ch.id ? "#fff" : C.black, margin: 0, fontFamily: "Poppins, sans-serif" }}>{ch.name.split(" ")[0]}</p>
          <p style={{ fontSize: 10, color: selected.id === ch.id ? "#aaa" : C.gray400, margin: 0 }}>Age {ch.age}</p>
        </div>
      </button>
    ))}
    <button style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: C.white, border: `1.5px dashed ${C.gray200}`, borderRadius: 50, cursor: "pointer", color: C.gray400, fontSize: 12, fontWeight: 600 }}>
      + Add Child
    </button>
  </div>
);
...
// ── Score Card ────────────────────────────────────────────────────────────────
const ScoreCard = ({ child }) => {
  const lvl = LEVEL_CONFIG[child.level] || LEVEL_CONFIG.Growing;
  const nextThreshold = lvl.threshold;
  const pct = Math.min(100, Math.round((child.gge_score / nextThreshold) * 100));

  return (
    <div style={{ margin: "0 16px 14px", background: C.black, borderRadius: 20, padding: "20px", overflow: "hidden", position: "relative" }}>
      {/* BG decoration */}
      <div style={{ position: "absolute", top: -20, right: -20, width: 120, height: 120, background: "rgba(255,255,255,0.03)", borderRadius: "50%" }} />
      <div style={{ position: "absolute", bottom: -30, left: -10, width: 90, height: 90, background: "rgba(255,255,255,0.02)", borderRadius: "50%" }} />

      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 44, height: 44, background: child.avatarColor, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 20, fontWeight: 900, color: "#fff" }}>{child.avatar}</span>
          </div>
          <div>
            <p style={{ fontSize: 16, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>{child.name}</p>
            <p style={{ fontSize: 11, color: "#666", margin: 0 }}>{child.role} • {child.district} • Active {child.lastActive}</p>
          </div>
        </div>
        <div style={{ background: lvl.bg, borderRadius: 10, padding: "4px 10px", border: `1px solid ${lvl.color}40` }}>
          <span style={{ fontSize: 11, fontWeight: 800, color: lvl.color }}>{child.level}</span>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 16 }}>
        {[
          { label: "GGE Score", value: child.gge_score, sub: "/1000" },
          { label: "Points", value: child.points.toLocaleString(), sub: "total" },
          { label: "Streak", value: `${child.streak}d`, sub: "active" },
          { label: "Certs", value: child.certsEarned, sub: "earned" },
        ].map(s => (
          <div key={s.label} style={{ background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: "10px 8px", textAlign: "center" }}>
            <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 17, fontWeight: 900, color: "#fff", margin: "0 0 1px" }}>
              {s.value}<span style={{ fontSize: 10, fontWeight: 500, color: "#666" }}>{s.sub}</span>
            </p>
            <p style={{ fontSize: 9, color: "#666", margin: 0, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
          <span style={{ fontSize: 11, color: "#888" }}>Progress to {lvl.next || "Max Level"}</span>
          <span style={{ fontSize: 11, fontWeight: 700, color: C.gold }}>{pct}%</span>
        </div>
        <div style={{ height: 6, background: "rgba(255,255,255,0.1)", borderRadius: 6, overflow: "hidden" }}>
          <div style={{ height: "100%", width: `${pct}%`, background: `linear-gradient(90deg, ${C.green}, ${C.gold})`, borderRadius: 6, transition: "width 0.8s ease" }} />
        </div>
      </div>

      {/* Weekly change */}
      {child.weeklyChange > 0 && (
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 12 }}></span>
          <span style={{ fontSize: 12, color: "#4ade80", fontWeight: 700 }}>+{child.weeklyChange} points this week</span>
        </div>
      )}
    </div>
  );
...
// ── Badges Section ────────────────────────────────────────────────────────────
const BadgesSection = ({ child }) => (
  <div style={{ margin: "0 16px 14px", background: C.white, borderRadius: 16, border: `1px solid ${C.gray100}`, padding: "16px" }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
      <p style={{ fontSize: 14, fontWeight: 800, color: C.black, margin: 0, fontFamily: "Poppins, sans-serif" }}>Badges</p>
      <span style={{ fontSize: 12, color: C.gray400 }}>{child.badges.filter(b => b.earned).length}/{child.badges.length} earned</span>
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
      {child.badges.map(b => (
        <div key={b.id} style={{ textAlign: "center", padding: "12px 8px", background: b.earned ? C.goldBg : C.gray50, borderRadius: 14, border: `1px solid ${b.earned ? C.gold + "40" : C.gray100}`, opacity: b.earned ? 1 : 0.5 }}>
          <span style={{ fontSize: 26, display: "block", marginBottom: 4, filter: b.earned ? "none" : "grayscale(100%)" }}>{b.icon}</span>
          <p style={{ fontSize: 10, fontWeight: 700, color: b.earned ? C.black : C.gray400, margin: "0 0 2px", lineHeight: 1.2 }}>{b.title}</p>
          <p style={{ fontSize: 9, color: C.gray400, margin: 0, lineHeight: 1.3 }}>{b.desc}</p>
        </div>
      ))}
    </div>
  </div>
...
const LearningSection = ({ child }) => (
  <div style={{ margin: "0 16px 14px", background: C.white, borderRadius: 16, border: `1px solid ${C.gray100}`, padding: "16px" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
      <p style={{ fontSize: 14, fontWeight: 800, color: C.black, margin: 0, fontFamily: "Poppins, sans-serif" }}>Learning Paths</p>
      <div style={{ background: C.greenBg, borderRadius: 8, padding: "3px 10px" }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: C.green }}>{child.lessonsCompleted} lessons done</span>
      </div>
    </div>
    {child.learningProgress.map((lp, i) => (
      <div key={i} style={{ marginBottom: i < child.learningProgress.length - 1 ? 14 : 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: C.black }}>{lp.path}</span>
          <span style={{ fontSize: 12, fontWeight: 700, color: lp.color }}>{lp.progress}%</span>
        </div>
        <ProgressBar value={lp.progress} color={lp.color} height={7} />
      </div>
    ))}
    {child.learningProgress.length === 0 && (
      <p style={{ fontSize: 13, color: C.gray400, textAlign: "center", padding: "12px 0", margin: 0 }}>No learning paths started yet</p>
    )}
  </div>
);
...
const QuickStats = ({ child }) => (
  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, margin: "0 16px 14px" }}>
    <div style={{ background: C.greenBg, borderRadius: 14, padding: "14px", border: `1px solid ${C.green}30` }}>
      <span style={{ fontSize: 22 }}></span>
      <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 24, fontWeight: 900, color: C.black, margin: "6px 0 2px" }}>{child.challengesCompleted}</p>
      <p style={{ fontSize: 12, color: C.green, fontWeight: 700, margin: 0 }}>Challenges Done</p>
    </div>
    <div style={{ background: C.blueBg, borderRadius: 14, padding: "14px", border: `1px solid ${C.blue}30` }}>
      <span style={{ fontSize: 22 }}></span>
      <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 24, fontWeight: 900, color: C.black, margin: "6px 0 2px" }}>{child.lessonsCompleted}</p>
      <p style={{ fontSize: 12, color: C.blue, fontWeight: 700, margin: 0 }}>Lessons Completed</p>
    </div>
  </div>
);

// ── Recent Activity ───────────────────────────────────────────────────────────
const ActivitySection = ({ child }) => (
  <div style={{ margin: "0 16px 14px", background: C.white, borderRadius: 16, border: `1px solid ${C.gray100}`, padding: "16px" }}>
    <p style={{ fontSize: 14, fontWeight: 800, color: C.black, margin: "0 0 12px", fontFamily: "Poppins, sans-serif" }}>Recent Activity</p>
    {child.recentActivity.map((a, i) => (
      <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", paddingBottom: i < child.recentActivity.length - 1 ? 12 : 0, marginBottom: i < child.recentActivity.length - 1 ? 12 : 0, borderBottom: i < child.recentActivity.length - 1 ? `1px solid ${C.gray100}` : "none" }}>
        <ActivityIcon type={a.type} />
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: C.black, margin: "0 0 2px", lineHeight: 1.35 }}>{a.text}</p>
          <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>{a.time}</p>
        </div>
        <span style={{ fontSize: 11, fontWeight: 800, color: C.green, background: C.greenBg, padding: "2px 8px", borderRadius: 6, flexShrink: 0 }}>+{a.pts}</span>
      </div>
...
// ── WhatsApp Share ────────────────────────────────────────────────────────────
const buildShareText = (child) => {
  const earnedBadges = child.badges.filter(b => b.earned).map(b => b.title).join(", ");
  return encodeURIComponent(
    ` *${child.name}'s GGE Progress Report* \n\n` +
    ` GGE Score: ${child.gge_score}/1000 (${child.level})\n` +
    ` Points: ${child.points.toLocaleString()}\n` +
    ` Streak: ${child.streak} days active\n` +
    ` Challenges: ${child.challengesCompleted} completed\n` +
    ` Lessons: ${child.lessonsCompleted} completed\n` +
    ` Certificates: ${child.certsEarned} earned\n\n` +
    ` Badges: ${earnedBadges || "Just getting started!"}\n\n` +
    ` +${child.weeklyChange} points this week!\n\n` +
    `_GGE — Guide, Grow, Earn • Free Forever_ `
  );
};

// ── AI Parent Tip ─────────────────────────────────────────────────────────────
const ParentTip = ({ child }) => {
    ...
    try {
      const res = await fetch("/api/groq", {
        ...
        body: JSON.stringify({
          messages: [{
            role: "user",
            content: `A parent is checking their child's GGE app progress. Child: ${child.name}, Age: ${child.age}, Level: ${child.level}, GGE Score: ${child.gge_score}/1000, Challenges done: ${child.challengesCompleted}, Lessons done: ${child.lessonsCompleted}, Streak: ${child.streak} days. Give the parent ONE practical tip (2-3 sentences) on how they can support their child's growth this week. Be warm, specific, and practical for an Indian family.`
          }],
          language: "english",
        }),
      });
      const data = await res.json();
      setTip(data.reply || "Encourage your child to complete one challenge every morning before school — consistency beats intensity.");
    } catch {
      setTip("Encourage your child to complete one challenge every morning before school. Small daily habits build big futures.");
    }
    setLoad(false);
  };
  ...
  return (
    <div style={{ margin: "0 16px 14px", background: C.black, borderRadius: 16, padding: "16px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: shown ? 12 : 0 }}>
        <span style={{ fontSize: 22 }}></span>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: "#fff", margin: 0 }}>AI Parent Tip</p>
          <p style={{ fontSize: 11, color: "#666", margin: 0 }}>Get personalised advice for your child</p>
        </div>
        {!shown && (
          <button onClick={getTip} style={{ padding: "7px 14px", background: C.green, border: "none", borderRadius: 10, fontSize: 12, fontWeight: 700, color: "#fff", cursor: "pointer" }}>
            Get Tip
          </button>
        )}
      </div>
      {shown && (
        loading
          ? <p style={{ fontSize: 13, color: "#666", margin: 0 }}>Getting personalised tip...</p>
          : <p style={{ fontSize: 13, color: "#ccc", margin: 0, lineHeight: 1.65 }}>{tip}</p>
      )}
    </div>
  );
};
...
const FamilyDashboardPage = ({ onBack, user }) => {
  const [selected, setSelected] = useState(MOCK_CHILDREN[0]);
  const [tab, setTab]           = useState("overview"); // overview | badges | activity
    ...
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Nunito, sans-serif" }}>
      ...
      <div style={{ background: C.black, padding: "16px 20px", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 17, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>Family Dashboard</p>
            <p style={{ fontSize: 11, color: "#666", margin: 0 }}>Real growth. Real progress. Free forever.</p>
          </div>
          <button onClick={shareWhatsApp} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", background: "#25D366", border: "none", borderRadius: 10, cursor: "pointer" }}>
            <span style={{ fontSize: 14 }}></span>
      ...
      {/* ── Child Selector ── */}
      <ChildSelector children={MOCK_CHILDREN} selected={selected} onSelect={setSelected} />

      {/* ── Score Card ── */}
      <ScoreCard child={selected} />
      ...
      <div style={{ display: "flex", gap: 6, padding: "0 16px 12px" }}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "9px", border: `1px solid ${tab === t.id ? C.black : C.gray200}`, borderRadius: 12, background: tab === t.id ? C.black : C.white, color: tab === t.id ? "#fff" : C.gray600, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ paddingBottom: 90 }}>
        {tab === "overview" && (
          <>
            <QuickStats child={selected} />
            <LearningSection child={selected} />
            <ParentTip child={selected} />
            {/* WhatsApp share card */}
            <div style={{ margin: "0 16px 14px", background: "#e7fbe7", borderRadius: 16, padding: "16px", border: "1px solid #a3d9a3", display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 30 }}></span>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: C.black, margin: "0 0 2px" }}>Share with Family</p>
                <p style={{ fontSize: 12, color: C.gray600, margin: 0 }}>Send {selected.name.split(" ")[0]}'s progress report via WhatsApp</p>
              </div>
              <button onClick={shareWhatsApp} style={{ padding: "8px 14px", background: "#25D366", border: "none", borderRadius: 10, cursor: "pointer", fontSize: 12, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                Share
              </button>
            </div>
          </>
        )}
        {tab === "badges"   && <BadgesSection  child={selected} />}
        {tab === "activity" && <ActivitySection child={selected} />}
      </div>
    </div>
  );
};
...
const CHALLENGES = [
  {
    id: 1, category: "Money", difficulty: "Easy", points: 50, duration: "Today",
    title: "Track Every Rupee",
    desc: "Write down every single expense you make today — chai, bus, lunch, everything.",
    proof: "Write your total spend for today in the box below.",
    tip: "Most Indians lose ₹200-500/month on untracked small expenses.",
    tag: "All",
  },
  {
    id: 2, category: "Health", difficulty: "Easy", points: 40, duration: "Today",
    title: "Walk 30 Minutes",
    desc: "Walk continuously for 30 minutes — morning or evening, your choice.",
    proof: "Where did you walk? How did you feel after?",
    tip: "30 min walking daily reduces diabetes risk by 30% — biggest problem in Tamil Nadu.",
    tag: "All",
  },
  {
    id: 3, category: "Skill", difficulty: "Medium", points: 100, duration: "3 Days",
    title: "Teach Someone Something",
    desc: "Teach one useful thing you know to a family member or neighbour this week.",
    proof: "Who did you teach? What did you teach them?",
    tip: "Teaching doubles your own understanding (Feynman technique).",
    tag: "All",
  },
  {
    id: 4, category: "Farming", difficulty: "Medium", points: 120, duration: "Today",
    title: "Check Soil Health",
    desc: "Test your soil by the bottle-shake method or check moisture with your finger 2 inches deep.",
    proof: "What did you find? Dry, moist, or waterlogged?",
    tip: "80% of crop failures in TN are from wrong watering — not weather.",
    tag: "Farmers",
  },
  {
    id: 5, category: "Career", difficulty: "Hard", points: 200, duration: "This Week",
    title: "Apply to 3 Jobs",
    desc: "Find and apply to 3 real jobs today — Naukri, LinkedIn, or local boards.",
    proof: "Paste the 3 job titles and company names you applied to.",
    tip: "Most job seekers apply to 1-2 jobs and wait. Top seekers apply to 10+ per week.",
    tag: "Job Seekers",
  },
  {
    id: 6, category: "Money", difficulty: "Hard", points: 180, duration: "This Week",
    title: "Save ₹100 This Week",
    desc: "Cut one non-essential expense and save ₹100 by end of this week.",
    proof: "What did you cut? How much did you save?",
    tip: "₹100/week = ₹5,200/year. That's a semester of books.",
    tag: "All",
  },
  {
    id: 7, category: "Community", difficulty: "Easy", points: 60, duration: "Today",
    title: "Help One Person",
    desc: "Do one concrete helpful act for someone today — carry bags, explain a form, share a resource.",
    proof: "What did you do and for whom?",
    tip: "Villages that help each other have 2x higher income growth than isolated ones.",
    tag: "All",
  },
  {
    id: 8, category: "Learning", difficulty: "Medium", points: 90, duration: "Today",
    title: "Read for 20 Minutes",
    desc: "Read anything useful for 20 minutes — newspaper, book, article, scheme details.",
    proof: "What did you read? What is one new thing you learned?",
    tip: "CEOs read 4-5 books/month. Average Indian reads 1.5 books/year.",
    tag: "Students",
  },
];

const LEADERBOARD = [
  { rank: 1, name: "Priya K.", district: "Coimbatore", points: 2840, badge: "Champion" },
  { rank: 2, name: "Murugan R.", district: "Madurai", points: 2610, badge: "Champion" },
  { rank: 3, name: "Deepa S.", district: "Chennai", points: 2390, badge: "Leader" },
  { rank: 4, name: "Karthik M.", district: "Salem", points: 2100, badge: "Leader" },
  { rank: 5, name: "Anitha P.", district: "Trichy", points: 1950, badge: "Leader" },
  { rank: 6, name: "Selvam T.", district: "Tirunelveli", points: 1720, badge: "Growing" },
  { rank: 7, name: "Kavitha N.", district: "Erode", points: 1580, badge: "Growing" },
  { rank: 8, name: "Vijay B.", district: "Vellore", points: 1340, badge: "Growing" },
  { rank: 9, name: "Radha M.", district: "Thanjavur", points: 1120, badge: "Growing" },
  { rank: 10, name: "Kumar S.", district: "Tiruppur", points: 980, badge: "Growing" },
];

const BADGE_COLORS = {
  Champion: { bg: "#fef3c7", text: "#d97706", border: "#fde68a" },
  Leader:   { bg: "#e6f4ec", text: "#1a6b3c", border: "#a7d7b8" },
  Growing:  { bg: "#eff6ff", text: "#2563eb", border: "#bfdbfe" },
};
...
// ── Difficulty Badge ──────────────────────────────────────────────────────────
const DiffBadge = ({ d }) => {
  const map = { Easy: { bg: "#e6f4ec", color: "#1a6b3c" }, Medium: { bg: "#fffbeb", color: "#d97706" }, Hard: { bg: "#fef2f2", color: "#b91c1c" } };
  const s = map[d] || map.Easy;
  return <span style={{ fontSize: 10, fontWeight: 700, background: s.bg, color: s.color, padding: "2px 8px", borderRadius: 6 }}>{d}</span>;
};

// ── Points Pill ───────────────────────────────────────────────────────────────
const PointsPill = ({ pts }) => (
  <span style={{ fontSize: 11, fontWeight: 800, background: C.black, color: "#f59e0b", padding: "3px 10px", borderRadius: 20, letterSpacing: 0.3 }}>
    +{pts} pts
  </span>
);

// ── Challenge Card ────────────────────────────────────────────────────────────
const ChallengeCard = ({ ch, onStart, completed }) => (
  <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${completed ? C.green + "60" : C.gray100}`, marginBottom: 12, overflow: "hidden", boxShadow: completed ? `0 0 0 2px ${C.green}30` : "none" }}>
    <div style={{ padding: "14px 16px 12px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8, marginBottom: 8 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: C.gray400, textTransform: "uppercase", letterSpacing: 0.8 }}>{ch.category}</span>
            <span style={{ fontSize: 10, color: C.gray200 }}>•</span>
            <span style={{ fontSize: 10, color: C.gray400 }}>{ch.duration}</span>
          </div>
          <p style={{ fontSize: 15, fontWeight: 800, color: C.black, margin: 0, fontFamily: "Poppins, sans-serif", lineHeight: 1.3 }}>{ch.title}</p>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
          <PointsPill pts={ch.points} />
          <DiffBadge d={ch.difficulty} />
        </div>
      </div>
      <p style={{ fontSize: 13, color: C.gray600, margin: "0 0 10px", lineHeight: 1.55 }}>{ch.desc}</p>
      <div style={{ background: C.gray50, borderRadius: 10, padding: "8px 12px", marginBottom: 10, borderLeft: `3px solid ${C.gold}` }}>
        <p style={{ fontSize: 11, fontWeight: 700, color: C.orange, margin: "0 0 2px" }}>WHY THIS MATTERS</p>
        <p style={{ fontSize: 12, color: C.gray600, margin: 0, lineHeight: 1.5 }}>{ch.tip}</p>
      </div>
      {completed ? (
        <div style={{ display: "flex", alignItems: "center", gap: 6, background: C.greenBg, borderRadius: 10, padding: "8px 12px" }}>
          ...
          <p style={{ fontSize: 13, fontWeight: 700, color: C.green, margin: 0 }}>Challenge Completed! Points earned.</p>
        </div>
      ) : (
        <button onClick={() => onStart(ch)} style={{ width: "100%", padding: "11px", background: C.black, color: C.white, border: "none", borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif", letterSpacing: 0.3 }}>
          Accept Challenge
        </button>
      )}
    </div>
...
// ── Submit Screen ─────────────────────────────────────────────────────────────
const SubmitScreen = ({ challenge, onSubmit, onBack }) => {
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/groq", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: [{ role: "user", content: `I completed the GGE challenge: "${challenge.title}". Here is my proof: "${proof}". Please give me a short motivational reply (2-3 sentences max) in a warm encouraging tone. Acknowledge what I did specifically.` }],
          language: "english",
        }),
      });
      const data = await res.json();
      setAiReply(data.reply || "Well done! You took real action today — that's what makes the difference.");
    } catch {
      setAiReply("Well done! Every small action builds a bigger life. Keep going!");
    }
    setLoading(false);
    setDone(true);
  ...
  if (done) return (
    <div style={{ padding: "0 16px", paddingBottom: 90 }}>
      <div style={{ textAlign: "center", padding: "40px 0 24px" }}>
        <div style={{ width: 72, height: 72, background: C.greenBg, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
        ...
        <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 22, fontWeight: 900, color: C.black, margin: "0 0 4px" }}>Challenge Done!</p>
        <p style={{ fontSize: 13, color: C.gray400, margin: 0 }}>+{challenge.points} points added to your score</p>
      </div>
      <div style={{ background: C.black, borderRadius: 16, padding: "18px 20px", marginBottom: 16 }}>
        <p style={{ fontSize: 11, fontWeight: 700, color: C.gold, textTransform: "uppercase", letterSpacing: 1, margin: "0 0 8px" }}>GGE AI Says</p>
        <p style={{ fontSize: 14, color: "#fff", lineHeight: 1.65, margin: 0 }}>{aiReply}</p>
      </div>
      <div style={{ background: C.greenBg, borderRadius: 14, padding: "14px 16px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <p style={{ fontSize: 12, fontWeight: 700, color: C.green, margin: "0 0 2px" }}>Points Earned</p>
          <p style={{ fontSize: 26, fontWeight: 900, color: C.black, fontFamily: "Poppins, sans-serif", margin: 0 }}>+{challenge.points}</p>
        </div>
        <div style={{ textAlign: "right" }}>
          <p style={{ fontSize: 12, color: C.gray400, margin: "0 0 2px" }}>Difficulty</p>
          <DiffBadge d={challenge.difficulty} />
        </div>
      </div>
      <button onClick={onBack} style={{ width: "100%", padding: 13, background: C.black, color: "#fff", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>
        Back to Challenges
      </button>
    </div>
  );
  ...
  return (
    <div style={{ padding: "0 16px", paddingBottom: 90 }}>
      <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.gray100}`, padding: "18px 16px", marginBottom: 14 }}>
        <span style={{ fontSize: 10, fontWeight: 700, color: C.gray400, textTransform: "uppercase", letterSpacing: 0.8 }}>{challenge.category} • {challenge.duration}</span>
        <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 17, fontWeight: 800, color: C.black, margin: "4px 0 8px" }}>{challenge.title}</p>
        <p style={{ fontSize: 13, color: C.gray600, margin: 0, lineHeight: 1.55 }}>{challenge.desc}</p>
      </div>
      <div style={{ background: C.black, borderRadius: 14, padding: "14px 16px", marginBottom: 14 }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: C.gold, margin: "0 0 4px" }}>PROOF REQUIRED</p>
        <p style={{ fontSize: 13, color: "#ccc", margin: 0, lineHeight: 1.5 }}>{challenge.proof}</p>
      </div>
      <textarea
        value={proof}
        onChange={e => setProof(e.target.value)}
        placeholder="Write your proof here... (minimum 10 characters)"
        rows={4}
        style={{ width: "100%", padding: "14px", borderRadius: 14, border: `1.5px solid ${proof.length > 10 ? C.green : C.gray200}`, fontSize: 14, fontFamily: "Nunito, sans-serif", resize: "none", outline: "none", background: C.white, color: C.black, boxSizing: "border-box", lineHeight: 1.55, marginBottom: 12 }}
      />
      <button
        onClick={submit}
        disabled={proof.trim().length < 10 || loading}
        style={{ width: "100%", padding: 13, background: proof.trim().length >= 10 ? C.green : C.gray200, color: proof.trim().length >= 10 ? "#fff" : C.gray400, border: "none", borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: proof.trim().length >= 10 ? "pointer" : "not-allowed", fontFamily: "Poppins, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
        {loading ? "Submitting..." : `Submit & Earn +${challenge.points} Points`}
      </button>
    </div>
  );
};
...
// ── Leaderboard Tab ───────────────────────────────────────────────────────────
const LeaderboardTab = ({ userPoints }) => (
  <div style={{ padding: "0 16px", paddingBottom: 90 }}>
    <div style={{ background: C.black, borderRadius: 16, padding: "18px 16px", marginBottom: 16, marginTop: 4 }}>
      <p style={{ fontSize: 11, fontWeight: 700, color: C.gold, textTransform: "uppercase", letterSpacing: 1, margin: "0 0 4px" }}>Your Score</p>
      <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 32, fontWeight: 900, color: "#fff", margin: "0 0 2px" }}>{userPoints.toLocaleString()}</p>
      <p style={{ fontSize: 12, color: "#888", margin: 0 }}>Complete challenges to climb the board</p>
    </div>
    <p style={{ fontSize: 13, fontWeight: 700, color: C.gray400, textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 10px" }}>Top 10 This Month</p>
    {LEADERBOARD.map((u, i) => {
      const isTop3 = i < 3;
      return (
        <div key={u.rank} style={{ background: C.white, borderRadius: 14, border: `1px solid ${isTop3 ? C.gold + "50" : C.gray100}`, padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: isTop3 ? RANK_COLORS[i] : C.gray100, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <span style={{ fontSize: 13, fontWeight: 900, color: isTop3 ? "#fff" : C.gray600 }}>{u.rank}</span>
          </div>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 14, fontWeight: 700, color: C.black, margin: "0 0 1px", fontFamily: "Poppins, sans-serif" }}>{u.name}</p>
            <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>{u.district}</p>
          </div>
          <div style={{ textAlign: "right" }}>
            <p style={{ fontSize: 15, fontWeight: 900, color: C.black, margin: "0 0 3px", fontFamily: "Poppins, sans-serif" }}>{u.points.toLocaleString()}</p>
            <span style={{ fontSize: 10, fontWeight: 700, background: BADGE_COLORS[u.badge]?.bg, color: BADGE_COLORS[u.badge]?.text, padding: "2px 8px", borderRadius: 6, border: `1px solid ${BADGE_COLORS[u.badge]?.border}` }}>{u.badge}</span>
          </div>
        </div>
      );
    })}
    <div style={{ background: C.greenBg, borderRadius: 14, padding: "14px 16px", marginTop: 8, textAlign: "center" }}>
      <p style={{ fontSize: 13, fontWeight: 700, color: C.green, margin: "0 0 2px" }}>Top 10 winners get real rewards</p>
      <p style={{ fontSize: 12, color: C.gray600, margin: 0 }}>Mobile recharge • GGE coins • GKFXL badge</p>
    </div>
  </div>
);

// ── Stats Bar ─────────────────────────────────────────────────────────────────
const StatsBar = ({ completed, totalPoints, streak }) => (
  <div style={{ display: "flex", gap: 8, padding: "12px 16px 0" }}>
    {[
      ...
      <div key={s.label} style={{ flex: 1, background: C.white, borderRadius: 12, padding: "10px 8px", textAlign: "center", border: `1px solid ${C.gray100}` }}>
        <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 18, fontWeight: 900, color: C.black, margin: "0 0 1px" }}>{s.value}</p>
        <p style={{ fontSize: 10, color: C.gray400, margin: 0, fontWeight: 600 }}>{s.label}</p>
      </div>
    ))}
  </div>
);

// ── Main ChallengeWinPage ─────────────────────────────────────────────────────
const ChallengeWinPage = ({ onBack, user }) => {
  const [tab, setTab]               = useState("challenges"); // challenges | leaderboard
  const [filter, setFilter]         = useState("All");
  const [activeChallenge, setActive] = useState(null);        // challenge being attempted
  const [completed, setCompleted]   = useState([]);           // array of completed IDs
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Nunito, sans-serif" }}>
      ...
      <div style={{ background: C.black, padding: "16px 20px", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <button onClick={activeChallenge ? () => setActive(null) : onBack} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "flex", alignItems: "center" }}>
            <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/>
            </svg>
          </button>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 17, fontWeight: 800, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>
              {activeChallenge ? activeChallenge.title : "Challenge & Win"}
            </p>
            {!activeChallenge && <p style={{ fontSize: 11, color: "#888", margin: 0 }}>Complete real challenges. Earn real rewards.</p>}
          </div>
          {!activeChallenge && (
            <div style={{ background: C.gold, borderRadius: 10, padding: "5px 10px" }}>
              <p style={{ fontSize: 12, fontWeight: 800, color: C.black, margin: 0 }}>{totalPoints} pts</p>
            </div>
          )}
        </div>
      </div>

      {/* ── If a challenge is active, show submit screen ── */}
      {activeChallenge ? (
        <div style={{ paddingTop: 12 }}>
          <SubmitScreen challenge={activeChallenge} onSubmit={handleSubmit} onBack={() => setActive(null)} />
        </div>
      ) : (
        <>
          {/* ── Stats ── */}
          <StatsBar completed={completed.length} totalPoints={totalPoints} streak={streak} />

          {/* ── Tabs ── */}
          <div style={{ display: "flex", padding: "12px 16px 0", gap: 8 }}>
            {["challenges", "leaderboard"].map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: "10px", border: "none", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", background: tab === t ? C.black : C.white, color: tab === t ? "#fff" : C.gray600, fontFamily: "Poppins, sans-serif", textTransform: "capitalize", border: `1px solid ${tab === t ? C.black : C.gray200}` }}>
          ...
          {tab === "challenges" && (
            <div style={{ padding: "12px 16px 0", paddingBottom: 90 }}>
              {/* Filter pills */}
              <div style={{ display: "flex", gap: 6, marginBottom: 14, overflowX: "auto", paddingBottom: 4 }}>
                {filters.map(f => (
                  <button key={f} onClick={() => setFilter(f)} style={{ flexShrink: 0, padding: "6px 14px", borderRadius: 20, border: `1.5px solid ${filter === f ? C.black : C.gray200}`, background: filter === f ? C.black : C.white, color: filter === f ? "#fff" : C.gray600, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>
                    {f}
                  </button>
                ))}
              </div>
              ...
              {/* Daily banner */}
              <div style={{ background: `linear-gradient(135deg, ${C.green}, #0d4a28)`, borderRadius: 16, padding: "16px 18px", marginBottom: 14, position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", top: -10, right: -10, width: 80, height: 80, background: "rgba(255,255,255,0.05)", borderRadius: "50%" }} />
                <p style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.6)", textTransform: "uppercase", letterSpacing: 1, margin: "0 0 4px" }}>Daily Reset</p>
                <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 16, fontWeight: 800, color: "#fff", margin: "0 0 4px" }}>New challenges every day</p>
                <p style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", margin: 0 }}>Complete all 8 today → Bonus 200 pts streak reward</p>
              </div>
              ...
              {filtered.length === 0 && (
                <div style={{ textAlign: "center", padding: "40px 20px", color: C.gray400 }}>
                  <p style={{ fontSize: 14 }}>No challenges for this filter.</p>
                </div>
              )}
            </div>
          )}
          ...
          {tab === "leaderboard" && (
            <div style={{ paddingTop: 12 }}>
              <LeaderboardTab userPoints={totalPoints} />
            </div>
          )}
        </>
      )}
...
const FeatureSubPage = ({ featureId, onBack, user }) => {
  ...
  if (featureId === "harvest")      return <DirectHarvestPage   onBack={onBack} user={user} />;
  if (featureId === "skillbazar")   return <SkillBazaarPage     onBack={onBack} user={user} />;
  if (featureId === "reels")        return <LifeReelsPage       onBack={onBack} user={user} />;
  if (featureId === "village")      return <VillageConnectPage  onBack={onBack} user={user} />;
  if (featureId === "family")       return <FamilyDashboardPage onBack={onBack} user={user} />;
  if (featureId === "challenge")    return <ChallengeWinPage    onBack={onBack} user={user} />;
