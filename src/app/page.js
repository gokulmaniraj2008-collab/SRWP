...
const CommunityGroupsTab = ({ user }) => {
  const [groups, setGroups] = useState(() => {
    ...
    return [
      { id: 1, name: "Tamil Nadu Farmers", desc: "For farmers across Tamil Nadu — crop tips, MSP updates, scheme alerts", members: 142, joined: false, district: "Tamil Nadu", ts: Date.now() - 86400000 * 5 },
      { id: 2, name: "GGE Students India", desc: "Students sharing study resources, exam tips and career guidance", members: 289, joined: false, district: "All India", ts: Date.now() - 86400000 * 3 },
      { id: 3, name: "Skill Sellers Network", desc: "List your skills, find buyers, share earnings tips — no middlemen", members: 74, joined: false, district: "All India", ts: Date.now() - 86400000 },
    ];
  });
  const [showCreate, setShowCreate] = useState(false);
  ...
  const sendGroupMsg = (groupId) => {
    ...
    setGroupChats(gc => ({ ...gc, [groupId]: [...(gc[groupId] || []), msg] }));
    setGroupMsg("");
  };
