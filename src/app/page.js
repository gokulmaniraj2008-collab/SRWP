...
// ── Home Feed Post Card ───────────────────────────────────────────────────────
const FeedPostCard = ({ post, user, onLike, onComment, onShare }) => {
  ...
  const tagColor = {
    Farmers: { bg: C.greenBg, color: C.green },
    All:     { bg: "#eff6ff", color: "#2563eb" },
    Earn:    { bg: "#fffbeb", color: "#d97706" },
    Rights:  { bg: "#fef2f2", color: C.red },
    Rural:   { bg: "#f5f3ff", color: "#7c3aed" },
    Students:{ bg: "#ecfeff", color: "#0891b2" },
    Parents: { bg: "#fdf4ff", color: "#a21caf" },
  };
  const tc = tagColor[post.tag] || tagColor.All;

  return (
    <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.gray100}`, marginBottom: 12, overflow: "hidden" }}>
      {/* Author row */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "13px 14px 10px" }}>
        <div style={{ width: 40, height: 40, borderRadius: "50%", background: C.black, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <span style={{ color: "#fff", fontWeight: 800, fontSize: 15, fontFamily: "Poppins, sans-serif" }}>
            {(post.author || post.users?.name || "U")[0].toUpperCase()}
          </span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: 0 }}>
              {post.users?.name || post.author || "GGE Member"}
            </p>
            {post.users?.gkfxl_badge && (
              <span style={{ fontSize: 9, fontWeight: 700, background: C.greenBg, color: C.green, padding: "1px 6px", borderRadius: 4 }}>GKFXL</span>
            )}
            {post.tag && (
              <span style={{ fontSize: 9, fontWeight: 700, background: tc.bg, color: tc.color, padding: "1px 7px", borderRadius: 4 }}>{post.tag}</span>
            )}
          </div>
          <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>
            {post.users?.district || post.district || "India"} · {ago(post.created_at || post.ts)}
          </p>
        </div>
      </div>
      ...
      {/* Image (if any) */}
      {post.image_url && (
        <div style={{ background: C.gray50, maxHeight: 220, overflow: "hidden" }}>
          <img src={post.image_url} alt="post" style={{ width: "100%", objectFit: "cover" }} />
        </div>
      )}

      {/* Stats row */}
      <div style={{ padding: "8px 14px", borderTop: `1px solid ${C.gray50}`, display: "flex", alignItems: "center", gap: 4 }}>
        <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>
          {(post.likes_count || 0) > 0 && `${post.likes_count} likes · `}
      ...
      <div style={{ borderTop: `1px solid ${C.gray50}`, display: "flex" }}>
        {[
          { icon: "heart",   label: post.liked ? "Liked" : "Like",   action: onLike,                      active: post.liked   },
          { icon: "message", label: "Comment",                         action: () => setShowComments(v => !v), active: showComments },
          { icon: "globe",   label: "Share",                           action: onShare,                     active: false        },
        ].map((btn, i) => (
          <button key={i} onClick={btn.action} style={{ flex: 1, padding: "10px 4px", border: "none", background: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, borderRight: i < 2 ? `1px solid ${C.gray50}` : "none" }}>
            <Icon n={btn.icon} size={16} color={btn.active ? (btn.icon === "heart" ? C.red : C.green) : C.gray400} />
            <span style={{ fontSize: 12, fontWeight: btn.active ? 700 : 500, color: btn.active ? (btn.icon === "heart" ? C.red : C.green) : C.gray400, fontFamily: "Nunito, sans-serif" }}>{btn.label}</span>
          </button>
        ))}
      </div>
      ...
      {/* Comments section */}
      {showComments && (
        <div style={{ borderTop: `1px solid ${C.gray50}`, padding: "10px 14px 12px" }}>
          {comments.slice(-3).map((c, i) => (
            <div key={c.id || i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              ...
              <div style={{ background: C.gray50, borderRadius: "4px 12px 12px 12px", padding: "7px 11px", flex: 1 }}>
          ...
          {comments.length > 3 && (
            <p style={{ fontSize: 11, color: C.green, fontWeight: 600, margin: "0 0 8px" }}>View all {comments.length} comments</p>
          )}
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            ...
            />
            <button onClick={sendComment} disabled={!commentText.trim() || sending} style={{ width: 38, height: 38, borderRadius: 10, background: commentText.trim() ? C.black : C.gray200, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: commentText.trim() ? "pointer" : "not-allowed", flexShrink: 0 }}>
              <Icon n="send" size={14} color={commentText.trim() ? "#fff" : C.gray400} />
            </button>
          </div>
        </div>
      )}
...
// ── Home Stories Strip ────────────────────────────────────────────────────────
const StoriesStrip = ({ user }) => {
  const items = [
    { label: "Your Story", isAdd: true },
    { label: "PM Kisan", color: "#1a6b3c" },
    { label: "Farming", color: "#d97706" },
    { label: "Career", color: "#2563eb" },
    { label: "Health", color: "#b91c1c" },
    { label: "Rights", color: "#7c3aed" },
  ];
  return (
    <div style={{ display: "flex", gap: 12, overflowX: "auto", padding: "12px 16px", background: C.white, borderBottom: `1px solid ${C.gray100}` }}>
      {items.map((s, i) => (
        <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, flexShrink: 0, cursor: "pointer" }}>
          <div style={{ width: 52, height: 52, borderRadius: "50%", background: s.isAdd ? C.gray50 : s.color, border: s.isAdd ? `2px dashed ${C.gray200}` : `2.5px solid ${s.color}`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
            {s.isAdd ? (
              <span style={{ fontSize: 22, color: C.gray400, lineHeight: 1 }}>+</span>
            ) : (
              <span style={{ color: "#fff", fontWeight: 800, fontSize: 17, fontFamily: "Poppins, sans-serif" }}>{s.label[0]}</span>
            )}
          </div>
          <p style={{ fontSize: 10, color: C.gray600, margin: 0, fontWeight: s.isAdd ? 500 : 600, textAlign: "center", maxWidth: 52, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.label}</p>
        </div>
      ))}
    </div>
  );
...
// ── Home Quick Feature Bar ────────────────────────────────────────────────────
const QuickFeatureBar = ({ setPage, setSubPage }) => {
  const items = [
    { icon: "zap",    label: "AI Guide",   action: () => setPage("ai")           },
    { icon: "book",   label: "Learn",      action: () => setSubPage && setSubPage("learning") },
    { icon: "target", label: "Challenge",  action: () => setSubPage && setSubPage("challenge") },
    { icon: "shield", label: "Schemes",    action: () => setSubPage && setSubPage("govbridge") },
    { icon: "map",    label: "Village",    action: () => setSubPage && setSubPage("village")  },
  ];
  return (
    <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "12px 16px 4px", background: C.bg }}>
      {items.map((item, i) => (
        <button key={i} onClick={item.action} style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, background: "none", border: "none", cursor: "pointer" }}>
          <div style={{ width: 48, height: 48, borderRadius: 14, background: C.black, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Icon n={item.icon} size={20} color="#fff" />
          </div>
          <p style={{ fontSize: 10, fontWeight: 600, color: C.black, margin: 0, fontFamily: "Nunito, sans-serif" }}>{item.label}</p>
        </button>
      ))}
    </div>
  );
...
// ── Home Page ─────────────────────────────────────────────────────────────────
const HomePage = ({ user, setPage, setSubPage }) => {
  const [posts, setPosts]         = useState([]);
  const [loading, setLoading]     = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [likedIds, setLikedIds]   = useState(new Set());
  const [filter, setFilter]       = useState("For You"); // For You | District | Following
  ...
  const SEED_POSTS = [
    { id: "seed1", author: "Priya R", users: { name: "Priya R", district: "Coimbatore", gkfxl_badge: true }, tag: "Farmers", text: "PM-KISAN amount credited today! ₹2000 received. If you haven't registered yet, go to pmkisan.gov.in — it takes only 10 minutes with Aadhaar.", likes_count: 24, created_at: new Date(Date.now() - 3600000).toISOString(), comments: [] },
    { id: "seed2", author: "Arjun M", users: { name: "Arjun M", district: "Chennai", gkfxl_badge: false },   tag: "Students", text: "Cleared my 12th board with 92%. First person in my village to score above 90. GGE AI helped me with study planning. Thank you GKFXL!", likes_count: 61, created_at: new Date(Date.now() - 7200000).toISOString(), comments: [] },
    { id: "seed3", author: "Lakshmi V", users: { name: "Lakshmi V", district: "Madurai", gkfxl_badge: false }, tag: "Earn", text: "Got my first freelance project through Skill Bazaar — ₹3500 for data entry work. No middleman. All 3500 came to me directly.", likes_count: 38, created_at: new Date(Date.now() - 86400000).toISOString(), comments: [] },
    { id: "seed4", author: "Murugan K", users: { name: "Murugan K", district: "Thanjavur", gkfxl_badge: true }, tag: "Farmers", text: "Soil test tip: Take a handful of soil, add water until it holds shape. If it stays — clay soil, hold less water. If it crumbles — sandy, needs more irrigation. Free method, no lab needed.", likes_count: 52, created_at: new Date(Date.now() - 172800000).toISOString(), comments: [] },
    { id: "seed5", author: "Kavitha S", users: { name: "Kavitha S", district: "Trichy", gkfxl_badge: false }, tag: "Rights", text: "Police stopped my brother without reason. We used the GGE Constitution page — Article 22 says they must tell the reason for arrest. They let him go in 30 minutes. Know your rights!", likes_count: 143, created_at: new Date(Date.now() - 259200000).toISOString(), comments: [] },
  ];
  ...
  const fetchPosts = async (isRefresh = false) => {
    ...
    try {
      let query = supabase
        .from("posts")
        .select("id, text, author, tag, image_url, likes_count, created_at, district, users(name, district, gkfxl_badge, is_admin)")
  ...
  const handleLike = async (post) => {
    ...
    setPosts(prev => prev.map(p => p.id === post.id
      ? { ...p, likes_count: (p.likes_count || 0) + (isLiked ? -1 : 1), liked: !isLiked }
      : p
    ));
    // Persist
    if (!post.id.toString().startsWith("seed")) {
      try {
        await supabase.from("posts").update({ likes_count: (post.likes_count || 0) + (isLiked ? -1 : 1) }).eq("id", post.id);
      } catch {}
    }
  };

  const handleShare = (post) => {
    const text = `${post.text}\n\n— via GGE (Guide, Grow, Earn) · Free for every Indian`;
  ...
  const submitPost = async () => {
    ...
    // Optimistic
    setPosts(prev => [{ ...newPostObj, id: `opt_${Date.now()}`, users: { name: user?.name, district: user?.district, gkfxl_badge: false } }, ...prev]);
    setNewPost("");
    setComposing(false);
    // Persist
    try {
  ...
  const filters = ["For You", "District", "Following"];

  return (
    <div style={{ fontFamily: "Nunito, sans-serif", background: C.bg, minHeight: "100vh", paddingBottom: 90 }}>
      {/* ── Top Bar ── */}
      <div style={{ background: C.black, padding: "12px 16px 14px", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: "50%", background: C.green, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
              {user?.photo
                ? <img src={user.photo} alt="profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : <span style={{ color: "#fff", fontWeight: 800, fontSize: 13, fontFamily: "Poppins, sans-serif" }}>{(user?.name || "U")[0].toUpperCase()}</span>
              }
            </div>
            <div>
              <p style={{ fontSize: 10, color: C.gray400, margin: 0 }}>Good {new Date().getHours() < 12 ? "morning" : new Date().getHours() < 17 ? "afternoon" : "evening"}</p>
              <p style={{ fontSize: 14, fontWeight: 700, color: "#fff", margin: 0, fontFamily: "Poppins, sans-serif" }}>{user?.name?.split(" ")[0] || "Friend"}</p>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {userCount && <span style={{ fontSize: 10, fontWeight: 700, background: C.green + "30", color: C.green, padding: "3px 9px", borderRadius: 6, border: `1px solid ${C.green}40` }}>{userCount.toLocaleString("en-IN")} members</span>}
            <button onClick={() => setPage("ai")} style={{ width: 34, height: 34, borderRadius: 10, background: C.green, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
              <Icon n="ai" size={16} color="#fff" />
            </button>
            <button onClick={() => setPage("community")} style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(255,255,255,0.1)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
              <Icon n="bell" size={16} color="#fff" />
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {filters.map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ flex: 1, padding: "7px 4px", border: `1px solid ${filter === f ? "#fff" : "rgba(255,255,255,0.15)"}`, borderRadius: 8, background: filter === f ? "#fff" : "transparent", color: filter === f ? C.black : "rgba(255,255,255,0.6)", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>
              {f}
            </button>
          ))}
        </div>
      ...
      {/* ── Quick Feature Bar ── */}
      <QuickFeatureBar setPage={setPage} setSubPage={setSubPage} />

      {/* ── Compose Box ── */}
      <div style={{ padding: "10px 16px 0" }}>
        <div onClick={() => setComposing(true)} style={{ display: "flex", alignItems: "center", gap: 10, background: C.white, border: `1.5px solid ${C.gray200}`, borderRadius: 12, padding: "11px 14px", cursor: "pointer", marginBottom: 4 }}>
          ...
          <p style={{ fontSize: 13, color: C.gray400, margin: 0, flex: 1 }}>Share with the GGE community...</p>
          <div style={{ background: C.black, borderRadius: 7, padding: "4px 10px" }}>
      ...
      {/* ── Compose Modal ── */}
      {composing && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 300, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setComposing(false)}>
          <div style={{ background: C.white, borderRadius: "20px 20px 0 0", padding: "20px 16px 40px", width: "100%", maxWidth: 480 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: C.black, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <span style={{ color: "#fff", fontWeight: 800, fontSize: 14 }}>{(user?.name || "U")[0].toUpperCase()}</span>
              </div>
              <div>
                <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: 0 }}>{user?.name}</p>
                <p style={{ fontSize: 11, color: C.gray400, margin: 0 }}>{user?.district || "India"}</p>
              </div>
            </div>
            <textarea
              autoFocus
              ...
              placeholder="What's happening in your area? Share a tip, question, or update..."
              style={{ width: "100%", minHeight: 110, padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${C.gray200}`, fontSize: 14, fontFamily: "Nunito, sans-serif", outline: "none", resize: "none", boxSizing: "border-box", color: C.black, background: C.gray50, lineHeight: 1.6 }}
            />
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
              <p style={{ fontSize: 12, color: newPost.length > 450 ? C.red : C.gray400, margin: 0 }}>{newPost.length}/500</p>
              <button
                onClick={submitPost}
                disabled={!newPost.trim() || posting || newPost.length > 500}
                style={{ background: newPost.trim() && !posting ? C.black : C.gray200, color: newPost.trim() && !posting ? "#fff" : C.gray400, border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}
              >
      ...
      <div style={{ padding: "10px 16px 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 13, fontWeight: 700, color: C.black, margin: 0 }}>
            {filter === "District" ? `${user?.district || "Your"} Feed` : filter === "Following" ? "Following" : "For You"}
          </p>
          <button onClick={() => fetchPosts(true)} disabled={refreshing} style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "none", fontSize: 11, fontWeight: 600, color: C.green, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>
            {refreshing ? <Spinner size={13} color={C.green} /> : <Icon n="grow" size={13} color={C.green} />}
            Refresh
          </button>
        </div>
        ...
        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[1, 2, 3].map(i => (
              <div key={i} style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.gray100}`, padding: "14px", height: 140, animation: "pulse 1.5s ease infinite" }}>
                <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: C.gray100 }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ height: 12, background: C.gray100, borderRadius: 6, marginBottom: 6, width: "40%" }} />
                    <div style={{ height: 10, background: C.gray100, borderRadius: 6, width: "25%" }} />
                  </div>
                </div>
                <div style={{ height: 10, background: C.gray100, borderRadius: 6, marginBottom: 6 }} />
                <div style={{ height: 10, background: C.gray100, borderRadius: 6, width: "80%" }} />
              </div>
            ))}
          </div>
        ) : (
          posts.map(post => (
            <FeedPostCard
              key={post.id}
              post={{ ...post, liked: likedIds.has(post.id) }}
              user={user}
        ...
        {/* Bottom CTA */}
        {!loading && (
          <div style={{ background: C.black, borderRadius: 16, padding: "18px 16px", marginTop: 8, textAlign: "center" }}>
            <p style={{ fontFamily: "Poppins, sans-serif", fontSize: 14, fontWeight: 800, color: "#fff", margin: "0 0 6px" }}>Guide. Grow. Earn.</p>
            <p style={{ fontSize: 12, color: C.gray400, margin: "0 0 14px" }}>Every Indian. Free forever.</p>
            <button onClick={() => setPage("guide")} style={{ background: C.green, color: "#fff", border: "none", borderRadius: 10, padding: "9px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Poppins, sans-serif" }}>
...
// ── Lazy-loaded real feature components (defined in separate files but inlined here for Next.js single-file compatibility) ──
// We dynamically import them to avoid circular deps and keep page.js self-contained.
// The actual component files (VillageConnect.js etc.) are used via dynamic import below.
const LazyVillageConnect  = typeof window !== "undefined" ? require("./VillageConnect").default  : null;
const LazyFamilyDashboard = typeof window !== "undefined" ? require("./FamilyDashboard").default : null;
const LazyChallengeWin    = typeof window !== "undefined" ? require("./ChallengeWin").default    : null;
const LazyLifeReels       = typeof window !== "undefined" ? require("./LifeReels").default       : null;
...
const FeatureSubPage = ({ featureId, onBack, user }) => {
  ...
  if (featureId === "village"  && LazyVillageConnect)  return <LazyVillageConnect  onBack={onBack} user={user} />;
  if (featureId === "family"   && LazyFamilyDashboard) return <LazyFamilyDashboard onBack={onBack} user={user} />;
  if (featureId === "challenge"&& LazyChallengeWin)    return <LazyChallengeWin    onBack={onBack} user={user} />;
  if (featureId === "reels"    && LazyLifeReels)        return <LazyLifeReels       onBack={onBack} user={user} />;
...
const CommunityGroupsTab = ({ user }) => {
  const [groupChats, setGroupChats] = useState({});   // { groupId: [{ author, text, ts }] }
  const [groupMsg,   setGroupMsg]   = useState("");

  const sendGroupMsg = (groupId) => {
    const text = groupMsg.trim();
    if (!text) return;
    const msg = { author: user?.name || "You", text, ts: Date.now() };
...
const MainApp = ({ user, onLogout }) => {
  ...
  return (
    <div style={{ fontFamily: "Nunito, sans-serif", background: C.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto" }}>
      {page === "home"      && <HomePage      user={user} setPage={setPage} setSubPage={setSubPage} />}
