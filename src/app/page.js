...
const ProblemsPage = ({ problems, setProblems, userId }) => {
  ...
  const submit = async () => {
    ...
    const savedDesc = form.desc;
    const savedSolution = form.solution;
    // Clear form immediately so user sees it reset
    setForm({ title: "", desc: "", solution: "" });
    let analysis = "";
    try {
      analysis = await callAI([{ role: "user", content: `Problem: ${savedTitle}\nDescription: ${savedDesc}\nKnown solution: ${savedSolution || "None"}` }],
    ...
    setProblems(p => [...p, { id: Date.now().toString(), title: savedTitle, desc: savedDesc, solution: savedSolution, analysis, votes: 0, votedBy: [], joinedBy: [], replies: [], teams: 0, tag: "General", status: "open", submittedBy: userId }]);
    setAnalyzing(false);
    setView("feed");
    setSuccessMsg("Problem submitted successfully!");
  ...
  return (
    <div style={{ padding: "24px 16px 100px" }}>
      ...
      {successMsg && (
        <div style={{ background: C.greenBg, border: `1px solid #86efac`, borderRadius: 10, padding: "12px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
          <Icon n="check" size={15} color={C.green} />
          <p style={{ fontSize: 13, fontWeight: 600, color: C.green }}>{successMsg}</p>
        </div>
      )}
      {view === "submit" ? (
