...
const ProblemsPage = ({ problems, setProblems, userId }) => {
  ...
  const submit = async () => {
    ...
    setProblems(p => [...p, { id: Date.now().toString(), title: form.title, desc: form.desc, solution: form.solution, analysis, votes: 0, votedBy: [], replies: [], teams: 0, tag: "General", status: "open", submittedBy: userId }]);
...
const AIGuidePage = () => {
  ...
  return (
    <div style={{ padding: "24px 16px 100px" }}>
      ...
      {error && <div style={{ background: C.redBg, border: `1px solid #fca5a5`, borderRadius: 10, padding: 12, marginBottom: 14 }}><p style={{ fontSize: 13, color: C.red }}>{error}</p></div>}
