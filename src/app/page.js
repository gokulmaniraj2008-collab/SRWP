...
const callAI = async (messages, systemPrompt) => {
  if (!messages?.length || !systemPrompt?.trim()) throw new Error("Missing AI input");
...
const db = {
  ...
  getProblems: async () => {
    if (!supabase) return [];
    const { data } = await supabase.from("problems").select("*, votes(*), teams(*), replies(*)").order("created_at", { ascending: false });
    return (data || []).map(p => ({
      ...p, desc: p.description,
      votes: p.votes?.length || 0,
      votedBy: (p.votes || []).map(v => v.user_id),
      teams: p.teams?.length || 0,
      joinedBy: (p.teams || []).map(t => t.user_id),
      replies: (p.replies || []).map(r => ({ text: r.text, author: r.author, time: new Date(r.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) })),
    }));
  },
  addProblem: async (problem) => {
    if (!supabase) return null;
    const { data } = await supabase.from("problems").insert([{
      title: problem.title, description: problem.desc,
      solution: problem.solution, analysis: problem.analysis,
      tag: problem.tag || "General", status: "open",
      submitted_by: problem.submittedBy, submitted_by_name: problem.submittedByName,
    }]).select().single();
    return data;
  },
  vote: async (problemId, userId) => {
    if (!supabase) return false;
    const { error } = await supabase.from("votes").insert([{ problem_id: problemId, user_id: userId }]);
    return !error;
  },
  joinTeam: async (problemId, userId, userName) => {
    if (!supabase) return false;
    const { error } = await supabase.from("teams").insert([{ problem_id: problemId, user_id: userId, user_name: userName }]);
    return !error;
  },
  addReply: async (problemId, authorId, author, text) => {
    if (!supabase) return null;
    const { data } = await supabase.from("replies").insert([{ problem_id: problemId, author_id: authorId, author, text }]).select().single();
    return data;
  },
  upsertUser: async (user) => {
    if (!supabase) return;
    await supabase.from("users").upsert([{ id: user.id, name: user.name, email: user.email, last_seen: new Date().toISOString() }]);
  },
  getOnlineUsers: async () => {
    if (!supabase) return [];
    const fiveMinAgo = new Date(Date.now() - 5 * 60 * 1000).toISOString();
    const { data } = await supabase.from("users").select("*").gte("last_seen", fiveMinAgo);
    return (data || []).map(u => ({ ...u, online: true }));
  },
};
...
const HomePage = ({ user, setPage, problems }) => (
  <div style={{ padding: "24px 16px 100px" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
      <div>
        <p style={{ fontSize: 13, color: C.gray400 }}>Welcome back</p>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 24 }}>
      {[{ n: problems.length, l: "Problems" }, { n: problems.reduce((a, p) => a + (p.teams || 0), 0), l: "Teams" }, { n: problems.filter(p => p.status === "solved").length, l: "Solved" }].map((s, i) => (
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
      {[{ icon: "target", title: "Share Problem", desc: "Post a real problem you face" }, { icon: "users", title: "Form Team", desc: "Min 2 members join together" }, { icon: "lightbulb", title: "Discuss", desc: "Vote and decide on solution" }, { icon: "zap", title: "AI Prompt", desc: "Get custom AI prompt" }, { icon: "github", title: "Build", desc: "Generate code with Claude" }, { icon: "globe", title: "Deploy", desc: "Go live on Vercel" }].map((h, i) => (
        <div key={i} style={{ background: C.white, borderRadius: 12, padding: 14, border: `1px solid ${C.gray100}` }}>
          <div style={{ width: 32, height: 32, background: C.gray50, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 8 }}><Icon n={h.icon} size={15} /></div>
...
// ── Problems ──────────────────────────────────────────────────────────────────
const ProblemsPage = ({ problems, setProblems, user, loadProblems }) => {
  ...
  const showAction = (msg) => { setActionMsg(msg); setTimeout(() => setActionMsg(""), 2500); };
  ...
  const submit = async () => {
    ...
    let analysis = "";
    try {
      analysis = await callAI([{ role: "user", content: `Problem: ${saved.title}\nDescription: ${saved.desc}` }],
        "Analyse this real-world problem in 2-3 sentences. Identify who is affected, why it matters, and the category.");
    } catch { analysis = "Analysis unavailable."; }
    const newP = { title: saved.title, desc: saved.desc, solution: saved.solution, analysis, submittedBy: user.id, submittedByName: user.name };
    const saved_db = await db.addProblem(newP);
    if (saved_db) {
      await loadProblems();
    } else {
      // Fallback if no supabase
      setProblems(p => [...p, { ...newP, id: Date.now().toString(), votes: 0, votedBy: [], teams: 0, joinedBy: [], replies: [], tag: "General", status: "open" }]);
    }
    setAnalyzing(false);
    setView("feed");
    setSuccessMsg("Problem submitted!");
    setTimeout(() => setSuccessMsg(""), 3000);
  };

  const vote = async (id) => {
    const p = problems.find(x => x.id === id);
    if (!p || (p.votedBy || []).includes(user.id)) return;
    const ok = await db.vote(id, user.id);
    if (ok) await loadProblems();
    else setProblems(ps => ps.map(x => x.id === id ? { ...x, votes: x.votes + 1, votedBy: [...(x.votedBy || []), user.id] } : x));
    showAction("Vote recorded!");
  };

  const joinTeam = async (id) => {
    const p = problems.find(x => x.id === id);
    if (!p || (p.joinedBy || []).includes(user.id)) return;
    const ok = await db.joinTeam(id, user.id, user.name);
    if (ok) await loadProblems();
    else setProblems(ps => ps.map(x => x.id === id ? { ...x, teams: x.teams + 1, joinedBy: [...(x.joinedBy || []), user.id] } : x));
    showAction("Joined team!");
  };
  ...
  const sendReply = async (id) => {
    ...
    const text = reply;
    setReply("");
    const saved = await db.addReply(id, user.id, user.name, text);
    if (saved) await loadProblems();
    else setProblems(ps => ps.map(p => p.id === id ? { ...p, replies: [...p.replies, { text, author: user.name, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }] } : p));
  };
  ...
  if (selected) {
    ...
    const hasJoined = (p.joinedBy || []).includes(user.id);
    return (
      <div style={{ padding: "16px 16px 100px" }}>
        {actionMsg && <div style={{ background: C.greenBg, border: `1px solid #86efac`, borderRadius: 10, padding: "10px 14px", marginBottom: 12, display: "flex", gap: 8 }}><Icon n="check" size={14} color={C.green} /><p style={{ fontSize: 13, fontWeight: 600, color: C.green }}>{actionMsg}</p></div>}
        <button onClick={() => setSelected(null)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: C.gray600, fontSize: 14, fontWeight: 600, marginBottom: 14, padding: 0 }}>
        ...
        {p.analysis && (
          <div style={{ background: C.blueBg, border: `1px solid #bfdbfe`, borderRadius: 10, padding: 12, marginBottom: 14 }}>
            <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}><Icon n="zap" size={13} color={C.blue} /><span style={{ fontSize: 11, fontWeight: 700, color: C.blue }}>AI Analysis</span></div>
            <p style={{ fontSize: 13, color: C.blue, lineHeight: 1.6 }}>{p.analysis}</p>
          </div>
        )}
        <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
          <button onClick={() => joinTeam(p.id)} disabled={hasJoined} style={{ flex: 1, padding: 11, background: hasJoined ? C.greenBg : C.black, color: hasJoined ? C.green : "#fff", border: `1px solid ${hasJoined ? C.green : "transparent"}`, borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: hasJoined ? "default" : "pointer" }}>
            {hasJoined ? "Joined ✓" : `Join Team (${p.teams})`}
          </button>
          <button onClick={() => vote(p.id)} disabled={hasVoted} style={{ padding: "11px 16px", background: hasVoted ? C.greenBg : C.gray50, border: `1px solid ${hasVoted ? C.green : C.gray200}`, borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: hasVoted ? "default" : "pointer", color: hasVoted ? C.green : C.black, display: "flex", alignItems: "center", gap: 5 }}>
            <Icon n="star" size={13} color={hasVoted ? C.green : C.gray600} /> {p.votes} {hasVoted ? "✓" : ""}
          </button>
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <h4 style={{ fontSize: 14, fontWeight: 700 }}>Replies ({(p.replies || []).length})</h4>
        </div>
        {(p.replies || []).length === 0
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => sendReply(p.id)} style={{ padding: "10px 14px", background: C.black, border: "none", borderRadius: 10, cursor: "pointer" }}><Icon n="send" size={15} color="#fff" /></button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "24px 16px 100px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={loadProblems} style={{ background: C.gray100, border: "none", borderRadius: 9, padding: "8px 10px", cursor: "pointer" }}><Icon n="refresh" size={15} /></button>
      ...
      {successMsg && <div style={{ background: C.greenBg, border: `1px solid #86efac`, borderRadius: 10, padding: "12px 14px", marginBottom: 14, display: "flex", gap: 8 }}><Icon n="check" size={15} color={C.green} /><p style={{ fontSize: 13, fontWeight: 600, color: C.green }}>{successMsg}</p></div>}
      {!supabase && <div style={{ background: C.amberBg, border: `1px solid #fcd34d`, borderRadius: 10, padding: "10px 14px", marginBottom: 14, display: "flex", gap: 8 }}><Icon n="info" size={14} color={C.amber} /><p style={{ fontSize: 12, color: C.amber }}>Add Supabase env vars for permanent storage</p></div>}
      {view === "submit" ? (
        <div style={{ background: C.white, borderRadius: 16, padding: 18, border: `1px solid ${C.gray100}` }}>
          ...
          <p style={{ fontSize: 13, color: C.gray400, marginBottom: 16 }}>AI will analyse it · Saved permanently</p>
          {[{ label: "Problem Title *", key: "title", ph: "What is the problem?", rows: 1 }, { label: "Describe in detail", key: "desc", ph: "Who is affected? Why?", rows: 3 }, { label: "Do you know any solution?", key: "solution", ph: "Optional", rows: 2 }].map(f => (
          ...
          <button onClick={submit} disabled={analyzing || !form.title.trim()} style={{ width: "100%", padding: 12, background: form.title.trim() ? C.black : C.gray200, color: form.title.trim() ? "#fff" : C.gray400, border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            {analyzing ? <><Spinner size={16} color="#fff" /> Analysing & Saving...</> : "Submit Problem"}
          </button>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {problems.length === 0 && <div style={{ textAlign: "center", padding: "48px 20px" }}><Icon n="target" size={36} color={C.gray200} /><p style={{ marginTop: 12, fontSize: 14, color: C.gray400 }}>No problems yet. Be the first.</p></div>}
          {problems.map(p => {
            const hasVoted = (p.votedBy || []).includes(user.id);
            return (
              <div key={p.id} style={{ background: C.white, borderRadius: 14, padding: 14, border: `1px solid ${C.gray100}`, cursor: "pointer" }} onClick={() => setSelected(p.id)}>
                ...
                <div style={{ display: "flex", gap: 4 }}><Icon n="users" size={13} color={C.gray400} /><span style={{ fontSize: 12, color: C.gray400 }}>{p.teams} team{p.teams !== 1 ? "s" : ""}</span></div>
              </div>
            );
          })}
        </div>
...
const CommunityPage = ({ problems, user }) => {
  ...
  useEffect(() => {
    db.getOnlineUsers().then(u => setOnlineUsers(u));
    db.upsertUser(user);
    const interval = setInterval(() => { db.upsertUser(user); db.getOnlineUsers().then(u => setOnlineUsers(u)); }, 30000);
    return () => clearInterval(interval);
  }, [user]);

  // Team chat realtime
  useEffect(() => {
    if (!chatTeam || !supabase) return;
    const roomId = `team_${chatTeam.id}`;
    supabase.from("messages").select("*").eq("room_id", roomId).order("created_at", { ascending: true })
      .then(({ data }) => setChatMessages((data || []).map(m => ({ text: m.text, author: m.author, time: new Date(m.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }))));
    const ch = supabase.channel(roomId).on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `room_id=eq.${roomId}` },
      p => setChatMessages(prev => [...prev, { text: p.new.text, author: p.new.author, time: new Date(p.new.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }])).subscribe();
  ...
  const sendTeamMsg = async () => {
    if (!chatText.trim() || !chatTeam) return;
    const text = chatText;
    setChatText("");
    if (supabase) await supabase.from("messages").insert([{ room_id: `team_${chatTeam.id}`, author: user.name, author_id: user.id, text }]);
    else setChatMessages(p => [...p, { text, author: user.name, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }]);
  };
  ...
  const sendDM = async () => {
    ...
    const text = dmText;
    setDmText("");
    const msg = { from_id: user.id, to_id: dmUser.id, text, author: user.name, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }), isMe: true };
    if (supabase) { await supabase.from("dm_messages").insert([{ from_id: user.id, to_id: dmUser.id, text }]); setDmMessages(p => [...p, msg]); }
    else setDmMessages(p => [...p, msg]);
  };

  const filtered = searchQuery.trim()
    ? onlineUsers.filter(u => u.name?.toLowerCase().includes(searchQuery.toLowerCase()) || u.id?.toLowerCase().includes(searchQuery.toLowerCase()))
    : [];
  ...
  if (dmUser) return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: C.bg }}>
      <div style={{ padding: "12px 16px", background: C.white, borderBottom: `1px solid ${C.gray100}`, display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        <button onClick={() => setDmUser(null)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}><Icon n="arrowLeft" size={20} /></button>
        <Avatar name={dmUser.name} size={34} online={true} />
        <div style={{ flex: 1 }}><p style={{ fontSize: 14, fontWeight: 700 }}>{dmUser.name}</p><p style={{ fontSize: 11, color: C.green }}>Online</p></div>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 140px", display: "flex", flexDirection: "column", gap: 8 }}>
        {dmMessages.length === 0 && <div style={{ textAlign: "center", paddingTop: 40 }}><Icon n="message" size={28} color={C.gray200} /><p style={{ marginTop: 10, fontSize: 13, color: C.gray400 }}>Start the conversation!</p></div>}
        {dmMessages.map((m, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: m.isMe ? "flex-end" : "flex-start" }}>
            {!m.isMe && <p style={{ fontSize: 10, fontWeight: 700, color: C.gray400, marginBottom: 2 }}>{m.author}</p>}
            <div style={{ maxWidth: "80%", padding: "10px 14px", borderRadius: m.isMe ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: m.isMe ? C.black : C.white, color: m.isMe ? "#fff" : C.black, border: m.isMe ? "none" : `1px solid ${C.gray100}`, fontSize: 14 }}>{m.text}</div>
      ...
      <div style={{ padding: "10px 16px", background: C.white, borderTop: `1px solid ${C.gray100}`, display: "flex", gap: 8, position: "fixed", bottom: 0, left: 0, right: 0, paddingBottom: "calc(10px + env(safe-area-inset-bottom, 0px))" }}>
        <input value={dmText} onChange={e => setDmText(e.target.value)} onKeyDown={e => e.key === "Enter" && sendDM()} placeholder={`Message ${dmUser.name}...`} style={{ flex: 1, padding: "11px 14px", borderRadius: 22, border: `1.5px solid ${C.gray200}`, fontSize: 14, outline: "none", background: C.gray50 }} />
        <button onClick={sendDM} style={{ width: 44, height: 44, borderRadius: "50%", background: dmText.trim() ? C.black : C.gray200, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><Icon n="send" size={16} color={dmText.trim() ? "#fff" : C.gray400} /></button>
      </div>
    </div>
  );
  ...
  if (chatTeam) return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: C.bg }}>
      <div style={{ padding: "12px 16px", background: C.white, borderBottom: `1px solid ${C.gray100}`, display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}><p style={{ fontFamily: "Syne, sans-serif", fontSize: 14, fontWeight: 700 }}>{chatTeam.title}</p><p style={{ fontSize: 11, color: C.gray400 }}>{chatTeam.teams} member{chatTeam.teams !== 1 ? "s" : ""} · Team Chat</p></div>
        <Tag label="Live" color="green" />
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 140px", display: "flex", flexDirection: "column", gap: 8 }}>
        {chatMessages.length === 0 && <div style={{ textAlign: "center", paddingTop: 40 }}><Icon n="message" size={28} color={C.gray200} /><p style={{ marginTop: 10, fontSize: 13, color: C.gray400 }}>No messages yet. Start the conversation!</p></div>}
        {chatMessages.map((m, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
              ...
              <div style={{ maxWidth: "78%", padding: "10px 14px", borderRadius: isMe ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: isMe ? C.black : C.white, color: isMe ? "#fff" : C.black, border: isMe ? "none" : `1px solid ${C.gray100}`, fontSize: 14 }}>{m.text}</div>
              <p style={{ fontSize: 10, color: C.gray400, marginTop: 2 }}>{m.time}</p>
            </div>
          );
        })}
      ...
      <div style={{ padding: "10px 16px", background: C.white, borderTop: `1px solid ${C.gray100}`, display: "flex", gap: 8, position: "fixed", bottom: 65, left: 0, right: 0, zIndex: 99 }}>
        <input value={chatText} onChange={e => setChatText(e.target.value)} onKeyDown={e => e.key === "Enter" && sendTeamMsg()} placeholder="Type a message..." style={{ flex: 1, padding: "11px 14px", borderRadius: 22, border: `1.5px solid ${C.gray200}`, fontSize: 14, outline: "none", background: C.gray50 }} />
        <button onClick={sendTeamMsg} style={{ width: 44, height: 44, borderRadius: "50%", background: chatText.trim() ? C.black : C.gray200, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><Icon n="send" size={16} color={chatText.trim() ? "#fff" : C.gray400} /></button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: "24px 16px 100px" }}>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        <div style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)" }}><Icon n="search" size={16} color={C.gray400} /></div>
        <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by name or @id..." style={{ width: "100%", padding: "11px 12px 11px 38px", borderRadius: 10, border: `1.5px solid ${C.gray200}`, fontSize: 14, outline: "none", boxSizing: "border-box" }} />
      </div>
      {filtered.length > 0 && (
        <div style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.gray100}`, overflow: "hidden", marginBottom: 16 }}>
          {filtered.map((u, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderBottom: i < filtered.length - 1 ? `1px solid ${C.gray100}` : "none" }}>
              <Avatar name={u.name} size={36} online={true} />
              <div style={{ flex: 1 }}><p style={{ fontSize: 14, fontWeight: 700 }}>{u.name}</p><p style={{ fontSize: 12, color: C.gray400 }}>@{u.id}</p></div>
              <button onClick={() => setDmUser(u)} style={{ background: C.black, color: "#fff", border: "none", borderRadius: 8, padding: "7px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>DM</button>
            </div>
          ))}
        </div>
      )}
      <div style={{ display: "flex", gap: 4, marginBottom: 18, background: C.gray100, borderRadius: 10, padding: 3 }}>
        {["teams", "online"].map(t => (
          <button key={t} onClick={() => setActiveTab(t)} style={{ flex: 1, padding: "8px", border: "none", borderRadius: 8, background: activeTab === t ? C.white : "none", fontWeight: activeTab === t ? 700 : 500, fontSize: 13, cursor: "pointer", boxShadow: activeTab === t ? "0 1px 4px rgba(0,0,0,0.08)" : "none" }}>
            {t === "online" ? `Online (${onlineUsers.length})` : `Teams (${teams.length})`}
          </button>
        ))}
      </div>
      {activeTab === "teams" && (
        <div>
          {teams.length === 0
            ? <div style={{ textAlign: "center", padding: "40px 20px" }}><Icon n="users" size={36} color={C.gray200} /><p style={{ marginTop: 12, fontSize: 14, color: C.gray400 }}>No active teams yet.</p></div>
            : teams.map((p, i) => (
                <div key={i} style={{ background: C.white, borderRadius: 14, padding: 14, border: `1px solid ${C.gray100}`, marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button onClick={() => { setChatTeam(p); setChatMessages([]); }} style={{ background: C.black, color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
          ...
          <div style={{ background: C.gray50, borderRadius: 16, padding: 18, textAlign: "center", border: `1px solid ${C.gray100}`, marginTop: 8 }}>
      ...
      {activeTab === "online" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {onlineUsers.length === 0 && <div style={{ textAlign: "center", padding: "40px 20px" }}><Icon n="users" size={36} color={C.gray200} /><p style={{ marginTop: 12, fontSize: 14, color: C.gray400 }}>No one online yet.</p></div>}
          {onlineUsers.map((u, i) => (
            <div key={i} style={{ background: C.white, borderRadius: 14, padding: 14, border: `1px solid ${C.gray100}`, display: "flex", alignItems: "center", gap: 12 }}>
              <Avatar name={u.name} size={42} online={true} />
              <div style={{ flex: 1 }}><p style={{ fontSize: 14, fontWeight: 700 }}>{u.name}</p><p style={{ fontSize: 12, color: C.green, fontWeight: 600 }}>Online now</p></div>
              <button onClick={() => setDmUser(u)} style={{ background: C.black, color: "#fff", border: "none", borderRadius: 8, padding: "7px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
...
const AIGuidePage = () => {
  ...
  const generatePrompt = async () => {
    ...
    try {
      const res = await callAI([{ role: "user", content: `Problem: ${problem}\nAnalysis: ${analysis}` }],
        `Write a detailed copy-paste ready prompt for Claude AI to build a complete website. Specify all pages, features, tech stack (Next.js + Tailwind + Supabase), and ask for complete working code. Start with "Build a..." or "Create a..."`);
  ...
  return (
    <div style={{ padding: "24px 16px 100px" }}>
      ...
      {error && error.trim() && (
        <div style={{ background: C.redBg, border: `1px solid #fca5a5`, borderRadius: 10, padding: 12, marginBottom: 14, display: "flex", gap: 8 }}>
      ...
      {step === 0 && (
        <div style={{ background: C.white, borderRadius: 16, padding: 18, border: `1px solid ${C.gray100}` }}>
          ...
          <textarea value={problem} onChange={e => { setProblem(e.target.value); setError(""); }} placeholder="e.g. Small farmers cannot find direct buyers..." rows={5} style={{ width: "100%", padding: "12px", borderRadius: 10, border: `1.5px solid ${C.gray200}`, fontSize: 14, resize: "none", outline: "none", boxSizing: "border-box" }} />
          <button onClick={analyse} disabled={loading} style={{ width: "100%", marginTop: 12, padding: 12, background: problem.trim() ? C.black : C.gray200, color: problem.trim() ? "#fff" : C.gray400, border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      {step === 2 && (
        <div>
          ...
          {[{ icon: "ai", title: "Use in Claude AI", desc: "Open Claude app → New chat → Paste prompt → Get code" }, { icon: "github", title: "Upload to GitHub", desc: "Create repo → Upload code files" }, { icon: "globe", title: "Deploy on Vercel", desc: "Import GitHub repo → Deploy → Live!" }, { icon: "check", title: "Fix Issues", desc: "Screenshot error → Upload to Claude → Get fix" }].map((s, i) => (
            <div key={i} style={{ background: C.white, borderRadius: 12, padding: 14, border: `1px solid ${C.gray100}`, marginBottom: 10, display: "flex", gap: 12 }}>
              <div style={{ width: 32, height: 32, background: C.black, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Icon n={s.icon} size={15} color="#fff" /></div>
  ...
  return (
    <div style={{ padding: "24px 16px 100px" }}>
      ...
      <div style={{ background: C.white, borderRadius: 16, padding: 16, border: `1px solid ${C.gray100}`, marginBottom: 20, display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {editing
            ...
            : <><p style={{ fontSize: 16, fontWeight: 700, marginBottom: 1 }}>{user.name}</p><p style={{ fontSize: 13, color: C.gray400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.email}</p></>
          }
        </div>
      ...
      <div style={{ background: C.greenBg, border: `1px solid #86efac`, borderRadius: 12, padding: "10px 14px", marginBottom: 20, display: "flex", gap: 8 }}>
        ...
        <p style={{ fontSize: 12, color: C.green, lineHeight: 1.5 }}>Groq API key stored securely on server. Never exposed to browser.</p>
      </div>
      ...
      <Section title="Support">
        ...
        <Row icon="info" label="About SRWP" value="v2.0.0 · Real · Supabase" onClick={() => setModal("about")} />
      </Section>
      <Section title=""><Row icon="logout" label="Log Out" danger onClick={() => setLoggedIn(false)} /></Section>
        ...
        <Sheet title="Notifications" onClose={() => setModal(null)}>
          {[{ l: "Email Alerts", d: "Get notified by email", k: "email" }, { l: "Push Notifications", d: "Browser alerts", k: "push" }, { l: "New Problems", d: "Alert on new posts", k: "newProblem" }].map(t => (
            <div key={t.k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: `1px solid ${C.gray100}` }}>
              ...
              <div onClick={() => setNotif(n => ({ ...n, [t.k]: !n[t.k] }))} style={{ width: 44, height: 24, borderRadius: 12, background: notif[t.k] ? C.black : C.gray200, cursor: "pointer", position: "relative", flexShrink: 0 }}>
      ...
      {modal === "privacy" && (
        <Sheet title="Privacy" onClose={() => setModal(null)}>
          {[{ t: "Your Data", d: "SRWP stores your name, email, and problems in Supabase. No data is sold." }, { t: "Chat Messages", d: "Stored in Supabase, visible to team members only." }, { t: "Delete Account", d: "Contact us to permanently delete all your data." }].map((r, i) => (
            <div key={i} style={{ padding: "12px 0", borderBottom: `1px solid ${C.gray100}` }}><p style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>{r.t}</p><p style={{ fontSize: 13, color: C.gray400, lineHeight: 1.6 }}>{r.d}</p></div>
          ))}
        </Sheet>
      )}
      {modal === "language" && (
      ...
      {modal === "help" && (
        <Sheet title="Help & FAQ" onClose={() => setModal(null)}>
          {[{ q: "How do I submit a problem?", a: "Tap Problems → Submit → describe your problem. AI analyses it and saves to database." }, { q: "How do teams work?", a: "Tap Join Team. Min 2 members. Then Message to chat in real-time." }, { q: "How does AI Guide work?", a: "Describe idea → Groq analyses → generates prompt → use in Claude to build website." }, { q: "Is SRWP free?", a: "Yes. Completely free for all users." }].map((f, i) => (
            <div key={i} style={{ padding: "12px 0", borderBottom: `1px solid ${C.gray100}` }}><p style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>{f.q}</p><p style={{ fontSize: 13, color: C.gray400, lineHeight: 1.6 }}>{f.a}</p></div>
          ))}
        </Sheet>
      )}
      {modal === "about" && (
        <Sheet title="About SRWP" onClose={() => setModal(null)}>
          ...
          {[{ l: "Version", v: "2.0.0" }, { l: "Framework", v: "Next.js 14" }, { l: "AI", v: "Groq llama-3.3-70b" }, { l: "Database", v: "Supabase (Real)" }, { l: "Hosting", v: "Vercel" }, { l: "Built by", v: "Gokul M" }].map((r, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: `1px solid ${C.gray100}` }}>
              <p style={{ fontSize: 13, color: C.gray400 }}>{r.l}</p><p style={{ fontSize: 13, fontWeight: 600 }}>{r.v}</p>
            </div>
          ))}
        </Sheet>
      )}
...
export default function Home() {
  ...
  useEffect(() => {
    if (loggedIn) {
      loadProblems();
      db.upsertUser(user);
      // Realtime problems subscription
      if (supabase) {
        const ch = supabase.channel("problems_feed")
          .on("postgres_changes", { event: "*", schema: "public", table: "problems" }, () => loadProblems())
          .on("postgres_changes", { event: "*", schema: "public", table: "votes" }, () => loadProblems())
          .on("postgres_changes", { event: "*", schema: "public", table: "teams" }, () => loadProblems())
          .subscribe();
        return () => supabase.removeChannel(ch);
      }
    }
  ...
  const pages = {
    ...
    problems: <ProblemsPage problems={problems} setProblems={setProblems} user={user} loadProblems={loadProblems} />,
    community: <CommunityPage problems={problems} user={user} />,
    ai: <AIGuidePage />,
    settings: <SettingsPage user={user} setUser={handleSetUser} setLoggedIn={handleLogout} />,
  };
  ...
  return (
    <div style={{ fontFamily: "DM Sans, sans-serif", background: C.bg, minHeight: "100vh", color: C.black, maxWidth: 480, margin: "0 auto" }}>
      ...
      {loadingProblems && page === "problems" && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, height: 3, background: C.black, zIndex: 999, animation: "none" }} />
      )}
